...
- **A knob lives in FIVE places or it is not armed where it matters** (rmbp 2026-09-22, the four-place trap fired on IOAPIC, KVBLANK, CAMERA1 and SDHCWRITE in one day): `unaos/arroyo` (knob → feature, a literal check leg), the kernel's `Cargo.toml` (the feature), `unaos/builder/src/main.rs` (the env→feature line — the x86 kernel a QEMU or metal boot RUNS is compiled from the builder's own map, and arroyo's `$KERNEL_FEATURES` never reaches it), `unaos/scripts/banner-cert.sh` (a row, or every armed media build exits 2 NO VERDICT) and `unaos/scripts/k8-reach.registry` (a row, or `check` reds `k8-reach UNREGISTERED`). A brief that names fewer files sends the executor to a DONE gate it cannot reach, and an armed run on a builder-built kernel with the line missing is vacuous: the banner names the feature and the artifact carries none of it. Enforcer: the KNOB→BUILDER WIRING CHECK and `knob-parity.sh` in `./arroyo check`; `banner-cert.sh`'s exit 2; this list in every brief that adds a knob.
- **Knob-off identity cannot certify a change to unconditional code, and asking for it is asking for two things that cannot both be true** (rmbp 2026-09-22: RENDCONV M2 refused, FLAKEFIX's two owed fixes blocked, NEUTRAL by construction — LEDGER B144's collision). `knoboff` compares the OFF image; a fix in code compiled on every build moves that image by definition. The proof for such a change is the REPLAY on the board's own legs (and the census diff, e.g. `[PCI-STOR]` byte-identical but timestamps), stated in the brief as the gate; a brief that demands `knoboff … exit 0` for a default-image fix is wrong before it is sent. Enforcer: warning only; the brief author checks the touched fn's `cfg`.
- **Never edit a script while its own gate is running** (rmbp 2026-09-22, FLAKEFIX): bash reads a script by byte offset, so an edit to `unaos/arroyo` (or any `.sh` a running gate is executing) can silently corrupt the run in progress and the verdict it prints. Copy the script to the scratch dir and run the copy, or wait for the gate; a run whose script moved under it is VOID, never quoted. Enforcer: warning only; the seat's fold and gate scripts run from `~/unaos-bench/scratch/<seat>/` copies, never from the tree under test.
