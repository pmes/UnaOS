...
| B11 | **The Orin's drag is structurally unfed, and x86's feed is its own.** `wm::drag_route_tail` is the ONLY route from a pointer report into `wm::drag_motion` on aarch64 (`drag_motion` has no aarch64 name-caller). Measured in `main.rs` at `170c7f0f`: `render_service` (Pi) contains **2** `drag_route_tail` calls (`:5464`, `:5474`), `orin_render_service` contains **0**, `x86_render_service` contains **0** — x86 does not need one, it feeds `wm::drag_motion` directly from `arch/x86_64/syscall.rs:6730/:6913/:6919`. The third call site, `route_input_to_active_el0():3901`, is `cfg(desktop_firmware)` and so IS compiled under `tegradesk`, but only carries the EL0-app drain. So on the Orin no pointer motion can reach the drag machine through the render body at all — which is consistent with pi 7's `DRAG_MOVES == 0`, and with this seat's separate proof that a zero count means nothing reached `drag_motion_paced` (`drag_begin` clears `DRAG_PACE_LAST_MS` at `wm.rs:16500` and the coalesce branch is guarded by `last != 0`, so the FIRST report of every gesture is admitted unconditionally — no clock value can suppress it). **Correction to the peer reading: S7 step 1 does NOT close this.** Step 1 converts the PI member only; the Orin inherits the shared body — and with it the tail call it lacks — at step 2. Anyone flying step 1 expecting the drag to work will score a false red | rmbp | orin + pi benches | open — root cause located from code, unflown; needs an S-row id at the next LEDGER fold (orin is allocating S30 for the xHCI accounting gap; this one needs its own, NOT taken here to avoid a collision) | `main.rs` render-family body counts at `170c7f0f`; `wm.rs:16500`/`:16639`/`:16765`; feeder first grepped by **orin 15** 02:3xZ (NOT pi 7 — orin corrected their own attribution; pi 7 then verified it independently), Pi-is-green half by pi 7 (boot12: 3 `placed`, so gates 1–4 all pass there). **Fix candidates, both in `main.rs`: (a) one line-neutral `drag_route_tail(ev)` fold in `jd2_console_pump` (`:2808`, `cfg(all(tegra, aarch64))` — orin's own region, no grant needed) or (b) S7 step 2. ⚠ (a) carries a byte-identity hazard nobody has flagged: `:2808` is ~6,180 lines ABOVE the file tail, so a NAIVE line insertion there renumbers every `panic::Location` below it — breaking pi's knob-off baseline AND x86 identity, both green under `check`. It MUST be a line-neutral fold, statement before the first `//` (P7)** | — |
