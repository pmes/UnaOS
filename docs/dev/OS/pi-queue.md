...
· NEW 2026-09-15 (rmbp BANNERCERT2, measured — the Pi half of a shared aarch64 finding): **`ehcihid` is on every aarch64 media banner and in no aarch64 artifact**, `kernel8` included, because `drivers/mod.rs:9` gates the module on `target_arch = "x86_64"` while `arroyo:337` appends the feature default-on and `arm_features` strips its twin `kbdwit` but not it. (`kernel8` builds from the curated `K8_FEATS`, which does NOT carry `ehcihid`, so the Pi IMAGE is clean — but `esp-arm` in this tree is not, and the one-line fix in `arm_features` is shared code that re-hashes Pi media too.) REGISTERED in `scripts/banner-cert.sh` so no verb reds; `kernel8` now certifies its banner against `kernel8.img` on every build (GATE-BANNERCERT). THE PI HALF OF THE DECISION: accepting the strip shifts cargo's `-Cmetadata` and changes the `kernel8.img` sha once — a one-time break of every recorded Pi media hash. Rule with the orin seat, take the line, delete the registry row in the same commit. Full finding: trunk `docs/dev/QUEUE.md` §5, the 2026-09-15 BANNERCERT2 row.
