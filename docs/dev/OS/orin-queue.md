...
✓ Waits on: exec-orin27-usblun, exec-orin27-closefold; then ONE fold gate; then build, stage, card. STOP carried from cmd8: `bootdisk.rs tegra_sd_state` cannot say "absent (no card)" without a flag from sdmmc_tegra.rs — the SKIP line is the witness for now.
