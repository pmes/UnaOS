...
## NEEDS PETER
· **GPU ladder, the fork (A61, GA10B-RUNG5-BRIEF.md §6, folded ec20ee6a):** (Q1) LAWS §3 says proprietary blobs never; NVIDIA's Driver License Agreement permits redistributing the unmodified binaries alongside an OSI-licensed OS and restricts use to NVIDIA parts — the licence is not the blocker, our rule is. Amend it for one staged, unmodified, never-linked vendor firmware image carried with its agreement, or does never mean never (the ladder then ends at rung 4)? (Q2, only if Q1 is yes) rung 5a flown attended, power cut acceptable? (Q3) enumerate `/lib/firmware/nvidia/ga10b/` on a JetPack 6.x rootfs now, or nothing read until Q1 is ruled? (Q5) `bcr_ctrl=0x011` (BRFETCH FALSE) is an untried door with no theory — built as rung 4f by executor GA10B4E; fly it or not? (Q4 needs no ruling and is being built: rung 4e, the six DMA addresses written `pa >> 8` — the flown 4b may have pointed the ROM 512 GiB above DRAM.)
