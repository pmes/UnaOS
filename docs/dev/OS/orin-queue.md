...
· **The Orin UEFI does not list the re-imaged microSD (MBR: p1 0x0c FAT32 127 MiB + p2 0x7f UnaFS) as a bootable disk in the native slot** (Peter 2026-09-12 03:3xZ: "the orin does not see it as bootable disk"); the same firmware booted an MBR FAT32 card from the USB reader on the rung-4 flights, and the slot reads this very card (SS32G) at the kernel level. Hypothesis, untested: the SD-slot boot path expects the JetPack layout (GPT + ESP GUID). Job: make `esp-jetson-img` emit GPT with an ESP-typed p1 (CARDREADY-GEOM changes; media-writer C7 reads it from the image, so it follows). Until then the Orin boots from the USB reader; render13 flies that way with ONE reader.
...
## BRANCH RESCUE — orin-only executor branches on NO track (BRANCHCENSUS 2026-09-17, `docs/dev/evidence/branch-census-0917/VERDICTS.md`)
· NEW  RASTWIN-RESCUE  `exec-orin-rastwin` **28bcd764** (2026-09-13, *video: RASTWIN — repair ORIN-RASTGLASS (A67); park the 3D window on the desktop*): board-only `arch/aarch64/display_tegra.rs` (the `[orinrast]` block) + `rast_demo.rs` + `main.rs` + `08_VIDEO/rasterizer.md` + FIXTURE_FLAKES.md; tip is an ancestor of no track branch. FOLD: re-derive — 3 commits including a merge of `f164b6fd`; merge the branch, do not cherry-pick. ids A66-A68 keep.
· NEW  GA10B6-RESCUE  `exec-orin-ga10b6` **07c29a62** (2026-09-13, *ga10b: GA10B6 — the second triple and the mapping arm as two one-delta boots, and the announce/result counter that was lying*): board-only `ga10b_fw.rs` + `ga10b_ignite.rs` + `ga10b_probe.rs` + `arroyo` + `Cargo.toml` + the rung-5 brief and its scorer; on NO track. It is THE NEXT GPU BOOT, already staged behind `UNAOS_GA10B_PROBE5=3`. FOLD: `git cherry-pick 07c29a62`, ids A72-A74 -> A80-A82.
· NEW  GA10B7-RESCUE  `exec-orin-ga10b7` **0676ed4a** (2026-09-15, *docs: GA10B7 — the written finding: the BCR/BROM door is closed by measurement, no boot worth a cycle*): board-only docs — 176 lines of GA10B-RUNG5-BRIEF §8 + ledger A95 + the queue verdict; DOCS-LOST, the finding exists nowhere else and is the ladder's conclusion. FOLD: `git cherry-pick 0676ed4a`.
