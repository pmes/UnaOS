...
| KD6 `repoint` — write the armed pointer 0x6101E0 | `UNAOS_KEPLER_TAKEOVER` | **no token in the tree — the rung's code is gone** (see §7) | s15 boot 2, 2026-07-23: `repoint wrote=00016000 rb=00000200` … `verdict rb-stuck=no` | 0x6101E0 is a read-only armed-state readout; the write never took. Boot 1 of the same sitting never reached the code at all (`takeover-abort no-match` — the refuted EVO-mirror head-match gate sat upstream) | KD5 | **shut-out, code deleted** |
| KD7 `latch` — EVO assembly (0x640460) + UPDATE (0x640080) | `UNAOS_KEPLER_TAKEOVER` | `:: kdisp: latch pre asm=` (`kepler_display.rs:285`, n=1) — **pre-state read only; the arm and UPDATE writes are gone** (`update_reg` at `kepler_display.rs:280` is now dead) | s28, 2026-07-25: `gop-overlap=YES-RESULT-VOID surf2=01600000+01C20000 gop=00020000+01C20000`; `armed=00000200 shadow=00000200` at t=1 and t=5, i.e. `0x200 << 8` = the GOP framebuffer, unmoved | the scratch surface at VRAM `0x1600000` sat **inside** the firmware's own framebuffer, so s17's "first UnaOS pixels" were direct painting into the GOP FB, not a latch. Peter watched the graphic appear *before* `pm-step fill done` — a full latch cycle before the latch. Arming a Kepler head plausibly needs a core-channel **method**, not a bare MMIO poke, and no pushbuffer existed | KD5; a working pushbuffer (→ §2 KF-wall) | **shut-out** |
| KD8 `mirror-sp` — EVO core-channel method-mirror decode | `UNAOS_KEPLER_TAKEOVER`, plus the hard-coded `run_recon` at `kepler_display.rs:288` | `:: kdisp: mirror-sp cand off=` (`kepler_display.rs:325`, n=1) | s25, 2026-07-25: `0x46C = 01004000` → `SET_STORAGE bit24 LAYOUT=1 = PITCH (LINEAR)`, `pitch = 0x4000 = 16384 B/row`; `0x468 = 07080B40` = h1800 w2880 | — | KD7's window | **proven** — it retired the whole block-linear campaign (s19–s24) in one read |
| KD9 `fb-draw` — full-panel paint at the GOP origin | `UNAOS_KEPLER_TAKEOVER` | `:: kdisp: fb-draw cover=` (`kepler_display.rs:410`, n=1) | s29, 2026-07-25: `cover=exact`; Peter's photo "edge to edge, everything predicted, nothing missing" | — | KD8 (the pitch), KD2 (the base) | **proven** |
...
- **KD6** — nothing re-opens a read-only register *as a pointer*. But the rung's **code is gone**,
  which R19 forbids: restore it (or its constants and its verdict string) so a later arc can
  re-read 0x6101E0 as the armed-state *witness* it turned out to be.
...
| KF4 `sched-status` — CHAN_TABLE_ERROR / SCHED_STATUS read | `UNAOS_KEPLER_FIFO` | `:: kepler: sched-status post-init err=` (`kepler.rs:2360`, n=1); also `post-restore` (`:2403`) and `post-submit` (`:3391`) | s9, 2026-07-22: `pre-init err=0 stat=0` → `post-init err=0x00000002` → `post-submit err=0x00000002 stat=0x00000005` | — | KF3 | **proven** (the instrument works; the chip names its own refusal) |
| KF5 `WITNESS STRIPPED` — the wall itself | `UNAOS_KEPLER_FIFO` | `:: kepler: PFIFO_CHAN[1] pre-submit:` (`kepler.rs:2372`, n=1) · `:: kepler: WITNESS STRIPPED. Restoring inst_off+0x0C ::` (`kepler.rs:2376`, n=1) · `:: kepler: witness-rematch end err=` (`kepler.rs:3408`, n=1) | s43 (GR25 Boot A): `witness-rematch end err=00000002 stat=00000005 valid=00002000` — byte-identical to the s25 baseline, **tenth** confirmation | not a rung that fails; the rung that **measures** every other rung's failure | — | **open** — the standing wall |
| KF6 `USERD_SNOOP` write (candidate A) | `UNAOS_KEPLER_FIFO` | **no token in the tree — the rung's code is gone** (see §7) | s10, 2026-07-22: `USERD_SNOOP orig=0` → write 1 → witness FAILED, snoop restored, `err=2 stat=5`, discriminators 0, RAMFC untouched | writes to `0x2a1c` read back as zero on GK107. The sitting refused to call it "config writes don't stick" — `SUBFIFO_ENG_MASK` and `PLAYLIST_WR/LEN` demonstrably stick; this register is either absent on this part, write-gated, or not a boolean | KF4 | **shut-out, code deleted** |
...
- **KF6 / KF8 / KF9** — each was refuted *as a sufficient cause of the strip*, under a host that
  ...
  where a FECS context machine is being brought up** — that is R19's exact scenario. Their code
  must come back first (§7).
...
Knob: `UNAOS_IVB3D` (`gen7`, implies `intel-ivb`). Design of record `gen7.md`.
...
| R4 / R4b `claim` (GGTT PTE round-trip) | `UNAOS_IVB3D` | `:: gen7: r4 ggtt-claim first=` (`gen7.rs:2470`, n=1) | Boot Ab: **the GGTT PTE round-trip is proven on metal.** "Unowned" has two proven shapes — every pre-image zero, or every pre-image the firmware **scratch-fill** (one identical valid PTE whose frame is the BDSM stolen-memory base read from the host bridge *this boot*, uniform across window, both neighbours and six distant probes) | — | R3 (a *reachable* wake for the census; a **confirmed** wake for the write) | **proven** — the GT **fabric** answers even while the engine block refuses. That split is what the rest of the ladder is about |
| R5 `execute` (RCS ring, `MI_STORE_DATA_IMM`) | `UNAOS_IVB3D` | `:: gen7: r5 verdict=` (`gen7.rs:3642`, n=16) | flight 4, both boots: `r5 verdict=enable-void … ctl_wrote=00000001 ctl_readback=00000000` | **no forcewake hold was in force.** R3 releases its acquire *inside its own rung*, by design — R3's job was to measure the acquire, not to keep it — so by the time R5 wrote `RING_CTL`, nothing was held. R5's own `next=` named it: `STOP-RING_CTL-enable-did-not-latch-likely-forcewake-released-R6-must-hold-forcewake-and-rearm` | R4 | **shut-out — and R6 re-opened it by changing exactly that one condition** |
| R6 `rearm` (the same ring, under a **held** wake) | `UNAOS_IVB3D` | `:: gen7: r6 verdict=` (`gen7.rs:4457`, n=13) · `r6-sentinel-hit` (`gen7.rs:4302`, n=8) · the decisive negative `r6-enable-void-under-every-hold` (`gen7.rs:4316`, n=3) | **flight 4, both boots: `r6 verdict=r6-sentinel-hit by=mt … attempts=1/3`** — head 0→0x20, sentinel `5EED1234` landed | — | **R3's acquire kept open across the whole arm / submit / drain / disable / restore**; R4's proven-unowned window | **proven** — `gen7.md` §2's "pending metal" is superseded |
| R6's TLB-invalidation sub-rung (`GFX_FLSH_CNTL` 0x101008) | `UNAOS_IVB3D` | `:: gen7: r6 tlb verdict=` (`gen7.rs:4387`, n=1) · `tlb-flush-write-silent` (`gen7.rs:4366`, n=2) | flight 4: `tlb=tlb-flush-write-silent` + `reclaim=leaked pages=3 bytes=12288 reason=no-invalidation-evidence` | the register read `0` before **and** after the write — §2.6's "a self-clearing flush register and a non-decoding offset are indistinguishable here" case, exactly. The address and the write-any-value semantic are `[PINNED]` (IVB PRM Vol1 Part3 §1.2.21 p.191); the PRM gives **no** name, format or read decode. The reclaim invariant then correctly refuses the free: *an unpinned register is never the sole reason a page goes back to the heap* | R6 | **shut-out — and it makes `reclaim=freed` structurally unreachable on this part as coded.** Cost: r5 (2) + r6 (2) + r7 (3) = **7 pages / 28 672 B leaked per armed boot**, bounded and one-shot |
| R7 `blit` (BCS, `XY_SRC_COPY_BLT` 16×16×32bpp) | `UNAOS_IVB3D` | `:: gen7: r7 verdict=` (`gen7.rs:5506`, n=15) · `r7-blit-verified` (`gen7.rs:5297`, n=6) | **flight 4, both boots: `r7 verdict=r7-blit-verified … attempts=1/3 … best_dst_match=256/256`** at `[15744ms]` boot 1 (L1565) and `[12199ms]` boot 2 (L10467). `ctl_readback=00000001 head_post=00000040 … sentinel_post=0B75C0DE sentinel_hit=1 dst_match=256/256 dst_crc=D7994E3D src_crc=D7994E3D … iters=3 cyc=7396`. `ring_idle=1 drain_iters=0 drain_cyc=28 … settled=1` — `col=exec == col=drain = 256`, so the G1 STOP condition (drain below exec) is **excluded by measurement**, both boots | — | R6 (the held wake and the arm discipline); R4b (the proven-unowned window, extended by one slot) | **proven** — the pre-registered falsifier is satisfied field-for-field, and none of the four failure tripwires fired. `gen7.md` §2.7's "held dark" is superseded. The bytes-not-DWords pitch ruling is now **metal-proven** |

**What would change the verdict**

- **R2** — re-score it. Its failure was measured on an instrument flight 4 proved blind
  (`battery_moved=0/17` through a verified DMA). A rung that used "battery moved" as its wake proof
  would score a healthy boot dead. Re-run R2 **with a behavioural witness** (a ring arm, a
  sentinel) instead of the battery, and with the transition-test ack R3 onward already uses.
...
- **R6 TLB** — pin an alternative invalidation witness, or promote the bounded 7-page leak to
  documented-accepted and delete the `reclaim=freed` expectation from the falsifier. R8's call.
- **R7** — nothing. It passed. What it hands R8 is in its own words:
...
## 7. ⚠ R19 exposure — rungs whose code is no longer in the tree

R19's enforcement says the code and the knob are **KEPT**. Seven refuted rungs' code is gone.
Verified by `grep -rn -F '<name>' unaos/crates/kernel/src` returning **0** at `27716175`:

| rung | sitting it was refuted at | what is gone | why it matters |
| --- | --- | --- | --- |
| KF6 `USERD_SNOOP` | s10 | `USERD_SNOOP` — 0 hits | the "writes read as zero" question was never explained, only set aside |
| KF8 `PFIFO_FLUSH` | s12 | `flush-executed`, `PFIFO_FLUSH` — 0 hits each | the *flushable* form is refuted; a coherence step under a running FECS is a different experiment |
| KF9 `CTRL_ADDR TARGET` | s13 | `CTRL_ADDR` — 0 hits | twelve clean reversible writes' worth of apparatus |
| KD6 `repoint` 0x6101E0 | s15 | the rung's write and its `rb-stuck` verdict — 0 hits for `asm-stuck` | the register is now known to be the armed-state **witness**; a later arc wants to read it |
| KD7 latch arm + UPDATE | s28 | the write half: `update_reg` (`kepler_display.rs:280`) is declared and never used; `asm-stuck` / `armed-followed` verdicts — 0 hits | KD7 is blocked on §2, not refuted; when a pushbuffer exists this is the first thing to re-run |
| display parameter ladders | s24/s26 | `lin-step`, `bwpg` — 0 hits each | superseded by KD8, and correctly so — but "superseded" is a different word from "deleted" |
| `gop-overlap` detector | s28 | 0 hits | the detector that **found** the confound of the whole s18–s26 campaign |

Two more, not deletions but knobless hard-codes: `run_recon` (`kepler_display.rs:288`) is a literal
`false` and `do_takeover` (`kepler_display.rs:330`) a literal `true` — code kept, knob absent, so
neither can be re-armed from a media build without an edit.

This section is a finding, not an accusation: nothing here was trashed maliciously, and every
deletion happened before R19 existed (2026-09-06). It is recorded so the restoration is a job with
a list, and so no later reader mistakes "the token isn't in the tree" for "the rung never ran".
...
the flight-5 capture already names the offending register and nobody has read it back — no boot
...
`docs/dev/OS/rmbp-queue.md` (section "GPU LADDERS") carry them, plus the three jobs that need no
flight at all: GMUX-1 (read a line already on disk), SHUTRESTORE (§7), GEN7DOC (§4).

Seven more rungs (§7) cannot be re-flown at all without restoring deleted code.
