...
| KD14 `kdhead` — the per-head decode, BRACKETED | `UNAOS_KEPLER_KDHEAD` (`nvidia-kepler-kdhead`, implies `nvidia-kepler` + `nvidia-kepler-takeover`); **`UNAOS_BEAM` supplies the bracket** | `:: KDHEAD: bracket source=` (`kepler_display.rs`, n=2 — one arm per posture) · `:: KDHEAD: gop ` (n=1) · `heads_distinct=` (n=3) · `:: KDHEAD: head=` (n=1, the decode line) · `:: KDHEAD: end rung=KD14` (n=1) · the two non-verdicts `DARK-NOT-SCORED` (n=4) and `NO-BRACKET-NOT-SCORED` (n=3) — **code: landed 2026-09-15 behind `nvidia-kepler-kdhead`, default OFF, READ-ONLY (`writes=0`)** | **flights 8 and 9, 2026-09-16** (both boots, byte-identical): `:: KDHEAD: end rung=KD14 bracket=beamx86-census blocks=5 separated=2 decoded=1 agree=1 disagree=0 writes=0 ::`, with `:: KDHEAD: head=0 live=yes geom=2880x1800 surface=0x20000 pitch=16384 vs gop=2880x1800 0x20000 16384 -> AGREE :: block=mirror mismatch=none ::` | — nothing failed. `mirror` (0x640400/0x300) `heads_distinct=2/4 stable=4/4 verdict=SEPARATES-heads`; `headstat` (0x616000/0x800) `4/4` but `decode=none reason=no-cited-slicing`, by design; `armed100` (0x616100/0x800), `evocore`, `headval` all `heads_distinct=1/4 stable=3/3 verdict=COLLAPSED-stride-does-not-separate-heads-here`, so **sitting #4's stride inference is now an observation**. Bracket `live=[y,n,n,n]`, so heads 1–3 read `DARK-NOT-SCORED` and are not evidence either way. No block scored `0/4`, so `UNSCORABLE-all-words-volatile` never fired | **KD4** for the control read itself (`head[0] stat underflow=`, [METAL s11]) and **BEAMX86** for taking it: the rung does NOT re-sample, it reads the per-head census `beam_probe` publishes, so bracket and beam gate cannot disagree — without `UNAOS_BEAM=1` on the same boot it prints `bracket=absent`, withholds every decode, and the capture says on its own wire that it is not a statement about the hardware. Also KD1 (the class probe) and KD2 (the GOP base every decode is compared against) | **proven** — its own falsifier (`-> AGREE` on a live head) was met on metal, `writes=0`. **⚠ AND IT RE-OPENS KD3 BY ITS OWN CONTRACT AND BY KD4's PRECEDENT: `mirror`'s slicing is now pinned by observation, so KD3's `shut-out` row above is stale and is NOT changed here — a status change on another rung's row is a call this scoring pass did not take. Next session: re-read KD3's row against this one.** `docs/dev/evidence/rmbp-0916/flight8-9/FLIGHT8-9.md` §4 |
...
| KF9 `CTRL_ADDR TARGET` ×12 | `UNAOS_KEPLER_FIFO` + `UNAOS_KEPLER_CTRL_ADDR` | `:: kepler: ctrladdr pbdma{} pre=` / `try target=` / `RO?` / `restored rb=` (`kepler.rs`) — **code: the audit restored 2026-09-15 (SHUTRESTORE) behind `nvidia-kepler-ctrladdr`, default OFF; the per-TARGET channel-bringup re-run is RESTORED TOO, 2026-09-17 (CTRLBIND), behind `nvidia-kepler-ctrlbind` (`UNAOS_KEPLER_CTRLBIND=1`, implies `nvidia-kepler-ctrladdr`), default OFF — it is rung KF9b in the row below, and §7's KF9 entry now reads restored IN FULL** | **flight 11, 2026-09-22 — ASKED AND ANSWERED, and the answer is the same wall.** The KF9b apparatus below flew and produced twelve `witness=silent` rollups, one per (PBDMA, TARGET), every one `bind=ok … chan00=00002000 chan04=11000001 sched=err=00000002,stat=00000000` — bit-identical to flight 10's unarmed `PFIFO_CHAN[1] pre-submit: 00=00002000 04=11000001` baseline, and to `init`'s own thirteenth copy on the same boot. s13's `wrote=rb` census had established only that `CTRL_ADDR[1:0]` is WRITABLE; this establishes what that writability buys, and it is nothing. (s13, 2026-07-23, kept: every TARGET value 0..3 on every PBDMA `wrote=rb`, witness never latched, `WITNESS FAILED` on all 12 steps — with no bind in the loop to make that a reading about TARGET at all.) | all three PBDMAs read `pre=00000000 hi=00000000` at entry, one PBDMA at a time, clean evidenced restores between steps (s13 and flight 11 alike). Flight 11 adds the knob line `UNAOS_WC=1 UNAOS_KEPLER=1 UNAOS_KEPLER_TAKEOVER=1 UNAOS_KEPLER_FIFO=1 UNAOS_KEPLER_CTRL_ADDR=1 UNAOS_KEPLER_CTRLBIND=1`, Kepler owning the panel, and — **the condition this rung did NOT vary and R19 still names** — a host on which **FECS context ucode has never run** | KF4 | **shut-out by measurement; code RESTORED** — and the distinction from the row's earlier `shut-out` is the point. The witness latches on a CHANNEL VALIDATE; none of the twelve produced one; therefore the strip's cause is not CTRL_ADDR's TARGET, under the conditions beside it. Not "ruled out": the FECS-ucode condition of the §2 bullet below is unchanged and unvaried. The code and its knob stay (R19) |
| KF9b `ctrlbind` — KF9's per-TARGET CHANNEL BRINGUP re-run, the apparatus s13 had | `UNAOS_KEPLER_FIFO` + `UNAOS_KEPLER_CTRL_ADDR` + `UNAOS_KEPLER_CTRLBIND` | `:: kepler: ctrlbind begin chan00=` · `:: kepler: ctrlbind pbdma<i> target=<t> ctrl_addr=… bind=<ok\|fail> witness=<latched\|silent> chan00=… chan04=… sched=err=…,stat=… ::` (twelve, one per encoding) · `:: kepler: ctrlbind pbdma<i> target=<t> skipped reason=ctrladdr-readonly ::` · `… skipped reason=ctrladdr-absent ::` · `:: kepler: ctrlbind chan-restored 00=… verdict=<clean\|DIRTY> ::` (`kepler.rs`, inside the KF9 audit loop; the bringup itself is `kepler::try_bind_and_witness` at the file tail) — **code: landed 2026-09-17 behind `nvidia-kepler-ctrlbind`, default OFF** | **flight 11, 2026-09-22 — FLOWN, and the apparatus did exactly what it was built to do.** `:: kepler: ctrlbind begin chan00=00000000 chan04=00000000 inst_off=02000000 ::`, then twelve rollups, pbdma0-2 × target 0-3, every one `ctrl_addr=0000000<t> bind=ok witness=silent chan00=00002000 chan04=11000001 sched=err=00000002,stat=00000000`, then `:: kepler: ctrlbind chan-restored 00=00000000(want 00000000) 04=00000001(want 00000000) verdict=DIRTY ::`. **NO `skipped reason=` LINE ANYWHERE**: all twelve encodings read WRITABLE on this boot, so the in-code R19 gate let every one of the twelve run — the rung is scored on twelve attempts, not on some of them. The one `witness=latched` that would have outranked everything else on the boot did not happen | the same conditions as the KF9 row above (they are one boot). The rung's own preconditions were all MET rather than skipped, which is what makes 12/12 a census and not a sample | **DEPENDS ON, and the first is enforced IN CODE (R19, the KF28 precedent):** KF9's own `RO?` check reading WRITABLE for that (pbdma, target) on THIS boot — the rung runs inside the audit loop with the encoding live and prints `skipped reason=ctrladdr-readonly` otherwise, writing nothing; `UNAOS_KEPLER_FIFO` to reach the call site at all (it is inside `kepler::init`'s fifo leg); KF4 (the `sched-status` bracket it re-reads) and KF5 (the strip it is scored by) | **shut-out** — its hypothesis (that some TARGET encoding lets the channel validate) failed 12/12 under the conditions beside it. ⚠ **AND THE BOOT RETURNED A SECOND FINDING, in the restore rather than the experiment, now fixed in code (KF9CLOSE, 2026-09-22, ledger B133):** `chan04` came back `00000001` against an entry value of `00000000`. **No write of this rung ever carried bit 0** — the seam writes `+0x04=00000400`, and it is the seam's NEXT write, the VALID bind word (`0xC0000000` ored with `inst>>12`) into `+0x00`, that leaves `+0x04` reading `11000001` on all twelve. Bit 0 is CHIP-SET channel state, and the way it is told apart from the chip's other bits is that the restore's 0-write DID clear `11000000` and did not clear bit 0: it landed while the channel was still BOUND, because the restore wrote `+0x04` FIRST and released `+0x00` second — the bringup's "VALID written last" rule copied into its own undoing, where it is backwards. The restore now RELEASES FIRST and puts the status word back LAST, reads both, and prints `order=00-then-04` with a residue-NAMING verdict `<clean\|DIRTY-chanstate\|DIRTY>`, `DIRTY-chanstate` meaning `+0x04` is off in bit 0 alone. **Which of `clean` and `DIRTY-chanstate` the next boot prints is itself the finding** — the second would mean the release does not drop the bit either, i.e. a latch no write clears — and QEMU cannot say which, having no Kepler |
...
| KF27 `kfbind` — the PBDMA base itself, derived from PTOP instead of guessed | `UNAOS_KEPLER_FIFO` + `UNAOS_KEPLER_KFBIND` | `:: KFBIND: ptop pbdma=` · `:: KFBIND: pbdma[` · `:: KFBIND: userd ` · `:: KFBIND: skipped write=` · `:: KFBIND: verdict base=` · `:: KFBIND: end rung=KF27` (`kepler_fifo.rs`) — **code: landed 2026-09-15 behind `nvidia-kepler-kfbind`, default OFF, READ-ONLY (`writes=0`)** | **flights 8 and 9, 2026-09-16** (both boots, byte-identical): `:: KFBIND: verdict base=NEITHER-ANSWERS … ib_get 00000000->00000000 ZERO … writes=0 restored=n/a -> STILL-DARK under: … ::` and `:: KFBIND: end rung=KF27 ptop=derived base_cmp=neither fetch=still-dark ::`. **IB_GET (USERD +0x88) read for the first time in the campaign**; `:: KFBIND: ptop pbdma=4 runlists=[01CD] pbdma0_base=023000 entries=64 nonzero=16 poison=0 ::`, and CE-R1's `ce-ptop` rows agree with it **64/64 dwords on each boot** | the four PTOP-derived bases (023000/027000/02B000/02F000) read `BADF1100 POISON` in every dword and the three legacy bases (040000/042000/044000) read clean `00000000 ZERO`, so `answers=n` everywhere and CHID/ACTIVE were deliberately not decoded. The `under:` clause names each condition as a value this boot read: PTOP-derived base comparison as scored; **FECS context microcode NOT resident** (no ctx ucode ran before the submit); CHAN_CUR/CHAN_NEXT not host-populated by this rung; **PBDMA start/clock sequence NOT written** (`skipped write=pbdma_start_clock reason=uncited`, and `skipped write=pbdma_chan_bind reason=uncited` with it); runlist submitted via 0x2270/0x2274. NOT ruled out (R19) — with `ctx-ucode` absent, a still-dark IB_GET is a statement about the instrument's preconditions, not about the GK107 | KF1 (the `pbdma-count 3` population), KF5 (the witness that scores the boot), KF24 (BAR1 identity — `userd_off` is a physical page, so the channel control area reads are addressable at all), and **CE-R1** for the PTOP table's existence, which is itself `never-run`: if R1 reports `REFUTED-CLEANLY` this rung's derivation half dies with it and its legacy half still scores | **open** — flown, `writes=0`, and the reading it produced does not resolve the base question, so the ladder has not stopped using it. **KF28 is consequently still `never-run` and correctly so: its in-code R19 gate needs `DERIVED-WINS` or `BOTH-ANSWER` on the same boot and would print `skipped reason=kf27-base-unresolved`.** `docs/dev/evidence/rmbp-0916/flight8-9/FLIGHT8-9.md` §4 |
...
| KF29 `kfunwedge` — ⚠ THE SACRIFICIAL RUNG: fire the `0x409504` poison ON PURPOSE, then observe/clear PRING and re-read `cpuctl` in the SAME boot | `UNAOS_KEPLER_FIFO` + `UNAOS_KEPLER_KFUNWEDGE` | `:: KFUNWEDGE: begin` · `:: KFUNWEDGE: base class=` · `:: KFUNWEDGE: pre pring=` · `:: KFUNWEDGE: poke read=0x409504` · `:: KFUNWEDGE: observe ` · `:: KFUNWEDGE: clear write=` / `skipped write=pring_clear_` · `:: KFUNWEDGE: verdict base=` · `:: KFUNWEDGE: end rung=KF29` (`kepler_fifo.rs`, `mod unwedge`) — **code: landed 2026-09-15 behind `nvidia-kepler-kfunwedge`, default OFF. NOT read-only: ONE deliberate read of `0x409504` and up to two W1C write-backs of bits observed SET; `restored=IMPOSSIBLE`** | — never flown | — | **DEPENDS ON:** KF21 — which is `proven` and which is what makes this rung legal at all: the write side of `0x409504` has already been taken harmlessly (s41, `terminal-poke 0x409504 wr=0` → `[NVIDIA] Initialization complete`), so the only untested half left is the read, and KF21's own row is the one that asks for this boot. KF20 for the ⚠ silicon law (`falcon_microcode_spec.md` §5.4). **KF27 resolving to `DERIVED-WINS` or `BOTH-ANSWER` on THIS boot, enforced IN CODE (R19)** — the rung re-runs KF27's base ladder read-only and prints `skipped reason=kf27-base-unresolved` otherwise, poisoning NOTHING, because its verdict is filed in this table beside ten eliminations read at an unproven address. s33boot1 for every PRING register it reads: all four were read real on this part in that boot and none has ever been read across a fault we caused | **never-run** |

⚠⚠ **KF29 IS A SACRIFICIAL BOOT AND THE OPERATOR MUST BE TOLD BEFORE THE SITTING IS PLANNED.** The Kepler drives this machine's panel — the x86 compositor's ignition is the Kepler takeover — so wedging its PRI ring on purpose may take the glass until a **POWER CYCLE**, not a reboot. The serial capture is the deliverable; a dark screen is the expected cost, not a new failure. **Fly it LAST in a sitting and ALONE**: never with BEAMX86, KDHEAD, KF27 or KF28 aboard, because every verdict collected after this rung is void by §5.4 and each of those rungs would print its conditions string over a boot this one had already poisoned. The code enforces the ordering *inside* the boot (the call site is the last kepler statement before the terminal poke, so every proven read has completed and printed first); it cannot enforce the knob set, which is why the rule is written here, on the queue row, and in `KEPLER-METAL-LOG.md`'s PENDING entry. **A poison is not restorable** — there is no write that un-reads a read, so the rung reports `restored=IMPOSSIBLE` rather than a success echo that cannot fail; the two W1C write-backs are clears of latched fault bits and are never described as a restore. And ⚠ **the terminal poke's own datum (`falcon_microcode_spec.md` §10, which evidences that write by its being the boot's only access to the offset) is VOID on a KF29 boot** — the read came first, and the `fecs-ledger` line prints `504_read_idx`/`504_write_idx` so the capture states it.
...
- **KF6 / KF8 / KF9** — each was refuted *as a sufficient cause of the strip*, under a host that
  ...
  owed here but the flight. ⚠ **KF9 IS NOW HALF-CLOSED AND THE HALVES MUST NOT BE CONFUSED**
  (flight 11, 2026-09-22, KF9b): *does any TARGET encoding change the strip* is ANSWERED — twelve
  binds, twelve silent witnesses, and the witness latches on a channel validate, so the strip's
  cause is not CTRL_ADDR's TARGET. What this bullet claims is the OTHER half and it is untouched:
  flight 11 ran on the same never-ran-FECS host as s13, so the condition named here was not varied
  and KF9 stays in §9's twenty-one-rung R19 list on that condition ALONE.
...
- **KF21** — the un-wedge experiment needs a boot where the poison **deliberately** fires, then a
  PRING observe/clear and a `cpuctl` re-read in the same boot. ⚠ **KF29 is now landed against this
  line and it is the EXPERIMENT, not another instrument** — it is the one rung in this tree that
  deliberately breaks the hardware it measures, and it is a **SACRIFICIAL boot** (see KF29's row).
  The clear was never the missing piece: pull 30 (`0e26447e`) shipped exactly this observe-and-clear
  chain, flew at s34, and found all five probed offsets clean, so the un-wedge half never ran and
  the code was removed. What was missing was a wedge that fires **on purpose**, at a point in the
  boot where losing the unit costs nothing. KF29 is that, and its `-> UNWEDGED` arm would be worth
  more than any single offset's value: it would make every future probe of an unproven `0x409xxx`
  offset survivable.
...
| R8 `fb_blit` (BCS, `XY_SRC_COPY_BLT` 64×64×32bpp at the panel's pitch) | `UNAOS_IVB3D_R8` (`gen7r8`, implies `gen7`) | `:: gen7: r8 verdict=` (`gen7.rs:6780`, n=16) · `r8-fb-blit-verified` (`gen7.rs:6567`, n=9) · `r8-fb-blit-verified-spill` (`gen7.rs:6568`, n=3) · `r8-gated-on-r7` (`gen7.rs:5826`, n=2) · `:: gen7: r8 begin` (`gen7.rs:5806`, n=1) | **flights 8 and 9, 2026-09-16 — THE RUNG DID NOT RUN.** `:: gen7: r7 verdict=r7-blit-verified …` on both boots, so R19's precondition was met and R8 was allowed to start, then `:: gen7: r8 verdict=r8-refused-surface-too-large … writes=0 note=refused-not-trimmed-a-rung-that-shrinks-its-own-experiment-reports-a-verdict-about-a-different-experiment ::` and `:: gen7: r8 next=STOP-destination-surface-over-the-stated-ceiling-raise-R8_DST_MAX_BYTES-deliberately-or-shrink-the-rect ::` | **its own destination ceiling, and by exactly one row.** ⚠ **THIS ROW'S PITCH IS WRONG AND THAT IS THE CAUSE:** the panel is 16384 B/row, not the 7 680 B stated below — `:: gen7: r8 geometry … stride_px=4096 bpp=4 dst_pitch_bytes=16384 dst_rows=65 dst_pages=260 …`, confirmed independently by `:: KDHEAD: gop … pitch=16384 ::`. `gen7.rs` takes `dst_rows = R8_RECT_H + 1` (one deliberate slack row so an inclusive `X2`/`Y2` is counted as spill rather than running off the allocation) at the panel's own pitch and caps at `R8_DST_MAX_BYTES` = 1 MiB; 65 rows is 16384 B over that cap and 64 rows would hit it to the byte. The refusal is the rung behaving correctly (R19: a rung that shrinks its own experiment reports a verdict about a different experiment). The engine is NOT implicated — R7 blitted on the same boot | **R7's verdict on THIS boot must be `r7-blit-verified`** (R19; enforced in code, the call is at the tail of `blit()` and takes R7's own local verdict) · R6's held-wake discipline · R4b's proven-unowned window, widened to `1 + 4 + dst_pages` slots | **never-run** — it flew on flights 8 and 9 and REFUSED ITSELF, so no blit was attempted and there is no verdict about the geometry it exists to test. Repair is a deliberate `R8_DST_MAX_BYTES` raise plus this row's pitch correction, then re-fly: rmbp-ledger **B111**, `docs/dev/evidence/rmbp-0916/flight8-9/FLIGHT8-9.md` §4 and §7 finding 1. R7 proved the BCS blits in the smallest possible case — 16×16, origin (0,0), 64-byte pitch, ONE destination page. R8 removes all four degeneracies at once: the panel's real pitch (**16384 B on this machine, measured on flights 8/9; this row said 7 680 B and that error is why the rung refused itself**), a NON-ZERO origin (the top-right corner), and a multi-page GGTT surface on both the read and the write side. It adds the one witness R7 structurally could not have — `spill`, the dwords OUTSIDE the rectangle — which is what tells a correct blit from a blit that landed the right pixels in the wrong place, and which ANSWERS `gen7.md` §2.7's unresolved `X2/Y2` inclusive-or-exclusive residual by arithmetic (`spill ≈ pitch/4` = a pitch-unit error; `spill ≈ 64` = an inclusive X2). ⚠ It does **not** write the panel: §5 G4 proves the Kepler owns it and G7 is never-run, so a blit into the live scanout would prove nothing visible AND would be a write into another device's VRAM aperture — the destination is CPU-readable scratch CARRYING the panel's geometry, and the visible copy is deferred to the rung after the gmux switch persists. It does **not** gate on a forcewake ack decode, as R3's row requires |
...
| rung | knob | witness token (file:line, n) | last metal verdict | failed under | depends on | status |
| --- | --- | --- | --- | --- | --- | --- |
| G1 `probe-complete` — iGPU pipes/planes census | `UNAOS_IVB` | `:: igpu: probe-complete ::` (`igpu.rs:786`, n=1) | s6 boot 1b, 2026-07-22, reconfirmed every boot since: Pipes A/B/C `CONF=0`; Planes A/B/C `CNTR=0 SURF=0 STRIDE=0 LINOFF=0 TILEOFF=0`; `DP_A=0x1C` | — | `UNAOS_IVB` publishing BAR0 | **proven** — every iGPU display plane is off |
| G2 `TEARDOWN HUNT TRACE` — four-point firmware-teardown hunt | `UNAOS_IVB` | `:: igpu: TEARDOWN HUNT TRACE ::` (`igpu.rs:546`, n=1) | s8 boot 1, 2026-07-22: **Point-0 is ALL-DEAD too** — pipes/planes/PP_STATUS/PP_CONTROL/DPLL_A read `0x00000000` at all four points, first-instruction-adjacent bootloader entry included; `DP_A=0x1C` constant | the "firmware tears iGPU scanout down during our bootloader window" theory is **dead**: panel power and the PLL were never on at any observable instant. The CF8-failed-read caveat was itself refuted — a failed read would have zeroed `DP_A` too | G1 | **shut-out (the teardown theory); proven (the census)** |
| G3 `PROTOCOL PROVEN` — gmux indexed handshake | `UNAOS_GMUX_IGD` | `:: igpu: PROTOCOL PROVEN (version plausible)` (`igpu.rs:586`, n=1) | s10 boot 1, 2026-07-22: **version 3.2.19** via the 32-bit indexed read; `MAX_BRIGHTNESS=0x3FF` as a second proof. Gate PASSED | s9's attempt **failed under the 3×8-bit read variant** — the version self-test returned implausible tuples and the gate correctly held, printing raw bytes only. s8 had already flagged the shape: `idx_SWITCH` and `idx_POWER` returned identical bytes twice, the signature of a missing ready-wait between index write and value read. **A read-width error, not a protocol absence** | G1 | **proven** |
| G4 `SW_DISPLAY` — who owns the panel | `UNAOS_GMUX_IGD` | `:: igpu: SW_DISPLAY` (`igpu.rs:596`, n=1) | s10: **`SW_DISPLAY=0x03 (DISCRETE)`, `SW_DDC=0x02 (DISCRETE)`, `DISC_POWER=0x03 (ON)`**, stable at Boot **and** Kernel | — | G3 | **proven** — the Kepler dGPU owns the panel at every observed instant. This **formally reversed** sitting #5's gmux/iGPU redirect and made "iGPU-all-dead" the **expected** state rather than a paradox |
| G5 `igpu-dpy rung=00 census` | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=00 name=census` (`igpu.rs:1393`, n=1) | flight 10, 2026-09-17: `:: igpu-dpy: rung=00 name=census ok=1 bdsm=0x8BA00001 ggc=0x00000211 ggtt0=0x8BA00003 ggtt1=0x8BA01003 aux_ctl=0x014300C8 frmcnt=0x00000000 ::`, the same on flights 8 and 9 | flight 5 never reached it: G6 refused upstream | G6 | **proven** — and `aux_ctl=0x014300C8` is the datum GMUXDPCD carries to the rollup as `aux_div=0x0C8`: bits 10:0 are the divider every AUX transaction on this machine inherits from firmware, never a value this tree invents |
| G6 pre-switch gate | `UNAOS_GMUX_IGD` | `:: igpu-dpy: pre-switch state DDC=` (`igpu.rs:1334`, n=1) · `pre-switch-not-accepted` (`igpu.rs:1357`, n=1) | flight 5: `:: igpu: [GMUX] REFUSED: pre-switch-not-accepted (status: 0x00000000) ::` → `:: igpu-dpy: LADDER highest=00/10 name=harness ok=0 pending=0 gmux=UNTOUCHED why=pre-switch-not-accepted elapsed_ms=1 ::` | the gate demands `DDC == GMUX_DDC_DIS` **and** `READ_DISPLAY == GMUX_DISPLAY_DIS` **and both** EXTERNAL registers ∈ {`GMUX_EXTERNAL_DIS`, `GMUX_EXTERNAL_KEPLER_OWNED` (0x21)}. At least one read on that boot fell outside. **The capture already contains the answer** — `igpu-dpy: pre-switch state` prints at `igpu.rs:1247`, *before* the gate returns at `:1270`, so the offending register is named on the wire and has never been read back. **Settled on metal:** flights 8/9/10 all print `… SW_EXT=0x01 SW_EXT_ST=0x21 DISP=0x03 EXT=0x21 sw_ext_state=UNACCEPTED ext_state=kepler-owned gate=ACCEPT ::` | G3 | **proven** (was fixed-unflown) — GMUX-1 read the datum (`docs/dev/evidence/rmbp-0915/GMUX-1-PRESWITCH.md`: SW_EXT=0x01 was 0x40, the write target); GMUX-2 scores 0x41 (`gmux_preswitch_decode`, igpu.rs tail) and restores nothing it did not read as state; flies next |
| G7 gmux switch to IGD | `UNAOS_GMUX_IGD` | `:: igpu: [GMUX] switched DISPLAY, EXTERNAL, and DDC to IGD` (`igpu.rs:2122`, n=1) | **REACHED**, flights 8/9/10 (2026-09-16/17): the line prints and DDC echoes `0x01` on all three (`:: igpu: [GMUX] switch read-back mismatch … DDC=0x01 DISP=0x02 EXT=0x01 ::`). Flight 10 has no in-git evidence file yet (a mid-boot slice carries no boot anchor, so GATE-LEDGER refuses it); its lines are quoted verbatim here and in ledger A7/B125 | DISP/EXT do not echo and are advisory by design; DDC is the only proven echo, and it is the one the rung gates on | G6 | **proven** — the mux moves. The flight-5 note that it "does not switch at all" was true of flight 5 and is now superseded; the row it depended on (G6) accepts |
| G10 `pp` panel-power frame + post-switch settle | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=03 name=pp ok=1 pp_write=DECLINED why=pp-bits-uncited` (`igpu.rs:1549`, n=1) · `:: igpu-dpy: rung=03 name=pp SETTLE ms=` (`igpu.rs:1567`, n=1) | **unflown** (new, GMUXDPCD). What it will read is already half-known: flights 8/9/10 all show `PP_CONTROL_PCH=0xABCD0008`, `PP_STATUS_PCH=0x00000000`, `PCH_PP_ON_DELAYS=0x00000000`, `PCH_PP_OFF_DELAYS=0x00000000`, `PCH_PP_DIVISOR=0x00186904` | **it writes nothing, on purpose, and three independent legs hold that.** (i) *No citation.* Raising VDD means a named bit in `PCH_PP_CONTROL` (0xC7204) and this tree holds no legal bit map for it: `gen7.md` names the register once, only to say it is read-only; `gpu_spec.md` does not name it; `gen7.rs`'s `PCH_PP_CONTROL_KEY` cites `0xABCD` as an entropy PATTERN, not a semantic; the only bit naming in tree (`docs/dev/GEMINI/video/iGUI/LADDER-igpu-bringup.md` rung 2) is marked **TBV** against i915 `intel_pps.c` and names PRM Vol 3 Part 4 "Panel Power Sequencing" as the document still needed. (ii) *Metal says it is unnecessary.* Flight 8 reached `highest=05/10 name=end ok=1` — DPCD **and** a header-and-checksum-valid 128-byte EDID — with the PPS in that same state, so the sink answers AUX with the sequencer untouched. `PP_ON_DELAYS=0` is a third, independent blocker: the panel's T1..T12 are unprogrammed and firing the PPS on zero delays is the documented panel-damage path | G7 | **fixed-unflown** — the rung is the census + a `PP_SETTLE_MS`-bounded settle between the mux write and the first AUX attempt, and the declension is printed with its reason token so the missing citation is a wire fact, not a comment |
| G8 ladder rollup | `UNAOS_GMUX_IGD` | `:: igpu-dpy: LADDER highest=` (`igpu.rs:2230`, n=1) | flight 10, 2026-09-17: `:: igpu-dpy: LADDER highest=03/10 name=dpcd ok=0 pending=2 gmux=MATCH why=aux-timeout-error elapsed_ms=9 ::` — **`03/10 dpcd aux-timeout-error`**. Flight 9 the same; flight 8, one knob apart, `highest=05/10 name=end ok=1 pending=2 gmux=FAILED why=none elapsed_ms=10` | **the pair is not reproducible, and that is the finding.** Same machine, same register readings, and the boot that got FURTHER voted `gmux=FAILED`. Under GMUXDPCD's renumbering `03` is now `pp` and `dpcd` is `04`, so captures must key on `name=`, never on the number | all | **open** — the instrument now reports 6 of 10 at best. The line gained `pp_seen=`/`pp=`/`pp_settle_ms=`/`aux_port=DPA(0x64010)`/`aux_div=`/`dpcd_tries=`, appended after `elapsed_ms=` so every pre-existing field keeps its name, value and order. **GMUX7 appends two more, on the same discipline** — `pps=` (G12's verdict, hoisted so a `name=end ok=1` capture can never be misread as *the panel was powered and nothing happened*) and `pending_items=`, which NAMES the entries `pending=` has only ever counted. Flight 11 printed `pending=2` with nothing on the wire saying which two; the answer is fixed by construction and now printed: `gmux:SWITCH_DDC@0x28<-0x02,gmux:SWITCH_DISPLAY@0x10<-0x03`, in the order `DisplayUnwind::execute` replays them, with `SWITCH_EXTERNAL` (0x40) **deliberately absent** because `ext_restore_target` has no cited status-to-write map for the `0x21` this machine reads — that absence IS why the count is 2 and not 3. ⚠ **THE RUNGS RENUMBER AGAIN:** `link` takes 06, `pps-on` 07, `link-train-dry` 08 and `end` moves 06 → 09, so flight 11's `highest=06/10 name=end` and flight 12's `highest=06/10 name=link` are DIFFERENT rungs at the same number. Compare on `name=`, never on the number. ⚠ **AND GMUX8 RENUMBERS THEM ONCE MORE:** `pps-read` (07b) takes the `highest` slot **8**, so `link-train-dry` moves 8 → 9, `modeset-read` (08b) takes **10**, and `end` moves 9 → **10**. The ladder is therefore at its full nominal height for the first time and the rollup's `/10` denominator is exact rather than aspirational. The `end` step now changes `rung_name` only and does not bump `highest`, because 08b already carried it to the top; a completed run is still distinguished exactly as it always was, by `name=end ok=1`. The two new rungs are lettered (`07b`, `08b`) precisely so the four pre-existing `rung=NN` literals — 03, 04, 06, 07, 08 — keep their spellings and every flight-11-era `awk` on `name=` still matches |
| G9 iGPU BLT ring (console acceleration) | `UNAOS_IVB` | `:: igpu-blt: ring=absent why=no-active-surface` (`igpu.rs:979`, n=1) | flight 5 and every flight: `ring=absent why=no-active-surface — every iGPU display plane is off (gmux routes the panel elsewhere); CPU path carries the console` | it needs an **active iGPU display plane** to prove scanout extent, and G4 proves there is none while the Kepler owns the panel | G7 opening | **shut-out under "the Kepler owns the panel"** |
| G11 `link` — the sink's link capability and link status, read and printed decoded | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=06 name=link ok=1 window=DPCD(0x000-0x00F)` (`igpu.rs:1720`, n=1) · `:: igpu-dpy: rung=06 name=link CAP 000:` (`igpu.rs:1722`, n=1) · `:: igpu-dpy: rung=06 name=link STATUS addr=DPCD(0x202-0x207)` (`igpu.rs:1725`, n=1) | **unflown** (new, GMUX7). Its precondition was met by flight 11, 2026-09-22: `:: igpu-dpy: rung=03 name=pp SETTLE ms=210 budget=not-a-cited-T3 moved=0 pp_ctl=0xABCD0008->0xABCD0008 pp_sts=0x00000000->0x00000000 elapsed_ms=215 ::` then `:: igpu-dpy: rung=04 name=dpcd try=1/7 ok=1 dpcd_rev=0x11 elapsed_ms=226 ::` then `:: igpu-dpy: rung=06 name=end ok=1 …` (rollup `highest=06/10 … gmux=MATCH why=none elapsed_ms=231 … dpcd_tries=1`). **The ladder ended because it ran out of RUNGS, not out of sink** — which is the whole reason this rung exists | it writes nothing but the AUX transaction registers rungs 04 and 05 already write, so its write envelope is exactly rung 04's and its only failure mode is the AUX one. ⚠ **ONE DECODE IS DELIBERATELY WITHHELD, and that is the finding to carry forward.** The only `MAX_LINK_RATE` map in this tree (`docs/dev/GEMINI/video/iGUI/LADDER-igpu-bringup.md:318-319`) reads `0x0A`=1.62 and `0x14`=2.7 Gbps, names no encoding at all for `0x06`, and the same document marks its DP-side facts **NEEDS-VERIFICATION against the DP 1.1/1.2 spec** at :420-422. A link rate is the number that sets the PLL frequency (:177 of that doc), so the rung prints `rate_raw=` verbatim with `rate_decode=TBV-tree-table` instead of converting on an unverified table. Lane-status bit meanings are withheld the same way: `bits=uncited` | G10 | **fixed-unflown** — the rung is two native AUX reads over the window this tree already cites, and the two things it refuses to decode are named on the wire rather than guessed |
| G12 `pps-on` — the panel-power write, declined a SECOND time, with the missing document named | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=07 name=pps-on ok=1 pp_write=DECLINED why=pp-bits-uncited` (`igpu.rs:1773`, n=1) · `:: igpu-dpy: rung=07 name=pps-on MISSING doc=PRM-Vol3-Part4-Panel-Power-Sequencing` (`igpu.rs:1775`, n=1) | **unflown** (new, GMUX7). The GMUX7 brief built this rung CONDITIONALLY — raise VDD only if the tree carries a cited bit map — and the condition is still false | **three independent legs hold the decline, and the third one is NEW.** (i) *No citation*, exactly as G10 states it: `gen7.md` names `0xC7204` once and only to call it READ ONLY, `gpu_spec.md` does not name it, `gen7.rs`'s `PCH_PP_CONTROL_KEY` cites `0xABCD` as an entropy PATTERN rather than a semantic, and the one bit naming in tree is marked **TBV** against i915 `intel_pps.c` NAMING — which the clean-room rule forbids promoting to a citation. (ii) *Metal says it is unnecessary*: flights 8 and 11 both got DPCD and a checksum-valid EDID with `PP_CONTROL_PCH=0xABCD0008` and `PP_STATUS=0x00000000`. (iii) **NEW — THE T VALUES THE BRIEF WOULD HAVE DERIVED DO NOT EXIST IN ANYTHING THIS LADDER HAS EVER READ.** population = the DPCD cap window rung 06 reads (`0x000-0x00F`) plus the 128-byte EDID base block rung 05 reads; hits=0. The cap window is link capability and the EDID base block is timing and identification; neither carries a panel-power T value, and `PP_ON_DELAYS`/`PP_OFF_DELAYS` read `0x00000000` on this part on flights 8, 9, 10 and 11. A PPS fired on zero delays is the documented panel-damage path | G11 | **fixed-unflown** — ⚠ **THE DECLINE IS A TOKEN, NOT AN ABORT**, by the same R19 reasoning rung 03 already carries in its own doc-comment. The ladder stops WRITING here — no PPS write, no panel power, no link write on this build — while G13, which writes nothing at all, still flies. Returning an error instead would have made G13 structurally unreachable on every boot, which is the BRANCHCENSUS defect `docs/dev/QUEUE.md` §5 has just spent a commit convicting. The reason is not hidden by that choice: it rides the rung line AND the rollup, as `pps=DECLINED:pp-bits-uncited` |
| G13 `link-train-dry` — the transcription that makes the next mode-set a copy | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=08 name=link-train-dry ok=1 DRY writes=0` (`igpu.rs:1916`, n=1) · `:: igpu-dpy: rung=08 name=link-train-dry WOULD-SET rate=` (`igpu.rs:1918`, n=1) · `:: igpu-dpy: rung=08 name=link-train-dry WOULD-WRITE-DPCD` (`igpu.rs:1920`, n=1) · `:: igpu-dpy: rung=08 name=link-train-dry WOULD-TOUCH` (`igpu.rs:1921`, n=2) · `:: igpu-dpy: rung=08 name=link-train-dry NOT-IN-TREE` (`igpu.rs:1934`, n=1) | **unflown** (new, GMUX7) | **it cannot fail and it cannot black the panel, and both statements are STRUCTURAL rather than promises: the rung contains no `write_volatile` and no `dp_aux_transfer` call at all.** Its whole product is five printed lines — the link parameters it WOULD set (taken from G11's DPCD bytes, never invented), the DPCD addresses it WOULD write (`0x100` `0x101` `0x102` `0x103`-`0x106`, cited to `LADDER-igpu-bringup.md` rung 5 and carried with that document's own NEEDS-VERIFICATION mark), the eleven display registers it WOULD touch with their CURRENT values, and the registers a real mode-set needs that **this tree has no offset for**. The WOULD-TOUCH set is exactly the `regs` module and not one register wider, because the clean-room rule for this seat is *every register cited*. ⚠ **GMUX8 REWROTE THIS RUNG'S LAST LINE, and only that line, because leaving it would have put a falsehood on the wire.** GMUX7's version read `NOT-IN-TREE htotal hblank hsync vtotal vblank vsync transcoder dp_tp_ctl dp_tp_status link_m link_n — population=igpu.rs-regs-module(igpu.rs:5-93), hits=0`. Eight of those eleven are now cited constants in that same module (G14/G15 added them from IVB PRM Vol3 Pt3), so `hits=0` stopped being true and the `:5-93` span stopped bounding the module. The remaining three are not "still missing" either — they are absent from the SILICON's own public spec (G15), which is a stronger and different statement — so the line now carries **two** populations under two tokens: `NOT-IN-TREE (none) LANDED-BY-GMUX8 …` and `NOT-IN-IVB-PRM transcoder dp_tp_ctl dp_tp_status — population=IVB-PRM-V3Pt3+V3Pt4, hits=0`. Every other token of the rung — its id, its name, `DRY`, `writes=0`, `WOULD-SET`, `WOULD-WRITE-DPCD`, `WOULD-TOUCH`, `NOT-IN-TREE` — is byte-identical to GMUX7's, so a flight-12 `awk` still matches | G11 | **fixed-unflown** — the next executor's first hour is a shopping list with a citation requirement attached, not a hunt. It is gated on G12 and says so on its own line (`gated_on=pps=DECLINED:pp-bits-uncited`) |
| G14 `pps-read` — the PPS bit map G10 and G12 could not cite, CITED — and the panel's real power state decoded from it | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=07b name=pps-read ok=1 writes=0 sample=rung07` (`igpu.rs:1840`, n=1) · `… REG pp_status@` (`igpu.rs:1842`, n=1) · `… REG pp_control@` (`igpu.rs:1848`, n=1) · `… REG pp_on_delays@` (`igpu.rs:1855`, n=1) · `… REG pp_off_delays@` (`igpu.rs:1858`, n=1) · `… REG pp_divisor@` (`igpu.rs:1860`, n=1) · `… ROLLUP panel_powered_by_fw=` (`igpu.rs:1873`, n=1) | **unflown** (new, GMUX8). What it will decode is already known field by field from flights 8/9/10/11, which all read `PP_CONTROL=0xABCD0008`, `PP_STATUS=0x00000000`, `PP_ON_DELAYS=0x00000000`, `PP_OFF_DELAYS=0x00000000`, `PP_DIVISOR=0x00186904` | **THE DOCUMENT G10 NAMED AS "STILL NEEDED" AND G12 NAMED AS "MISSING" WAS FETCHED AND READ: Intel® OpenSource HD Graphics PRM Vol 3 Part 4 (South Display Engine Registers, Ivy Bridge), Doc Ref `IHD-OS-V3 Pt 4 – 05 12`, §2.4 pp.38–43**, from intel.com, transcribed into `gpu_spec.md` §6.2 with a section and page on every field. `i915`/`intel_pps.c` was not opened and the tree's own TBV map — sourced to i915 *naming*, which is exactly why GMUX7 refused to promote it — was not consulted. ⚠ **THIS DOES NOT UNDO G12'S DECLINE AND MUST NOT BE READ AS DOING SO.** It retires exactly ONE of G12's three legs, leg (i) *no citation exists*. Legs (ii) *metal says the write is unnecessary* (flights 8 and 11) and (iii) *the T values are unprogrammed, and a PPS fired on zero delays is the documented panel-damage path* are untouched, and this build still writes nothing to the PPS on any boot. **WHAT THE DECODE ALREADY PREDICTS, PRM against the bytes four flights have read:** `PP_CONTROL=0xABCD0008` is Write Protect Key = `ABCDh` (§2.4.2 p.40, *Disable Write Protect*) with bit 3 **eDP VDD Override for AUX = Force** — a bit the PRM says exists "to force on VDD for the embedded DisplayPort panel so AUX transactions can occur **without enabling the panel power sequence**". That is the state every AUX rung of this ladder has read the sink in, and it is the mechanism behind flight 11's `dpcd_tries=1` with `pp_sts=0x00000000` throughout: **firmware, not this ladder, is why AUX answers with the sequencer off.** **IT ALSO NAMES A TRAP FOR THE WRITE RUNG:** §2.4.3 p.41's workaround ties the `ABCD` key to `PP_ON_DELAYS[31:30] = 01b DisplayPort A`, while `PP_ON_DELAYS` reads `0x00000000` = port select **LVDS**. Firmware installed the DP-A key over an LVDS port select with zero delays, and `port_sel_conflict=key-ABCD-but-port-sel-not-DPA` is on the wire for it. **AND IT RETIRES A TREE CLAIM:** `gen7.rs`'s `PCH_PP_CONTROL_KEY` cites `0xABCD` as an entropy PATTERN with no semantic; §2.4.2 p.40 makes it a documented magic value with a documented effect | G12 | **fixed-unflown** — it writes nothing (`writes=0`), takes no second sample (`sample=rung07`, deliberately, so nobody diffs two reads as though motion had been measured), and answers the one question a future `pps-on` must not get wrong: `panel_powered_by_fw=` is what the write rung must **preserve** rather than re-do |
| G15 `modeset-read` — the dry mode-set, read off the running machine | `UNAOS_GMUX_IGD` | `:: igpu-dpy: rung=08b name=modeset-read ok=1 writes=0` (`igpu.rs:1995`, n=1) · `… PORT dp_a@` (`igpu.rs:1997`, n=1) · `… PIPECONF conf@` (`igpu.rs:2016`, n=1) · `… TIMING-H htotal@` (`igpu.rs:2033`, n=1) · `… TIMING-V vtotal@` (`igpu.rs:2042`, n=1) · `… SRCSZ srcsz@` (`igpu.rs:2047`, n=1) · `… MN1 datam1@` (`igpu.rs:2058`, n=1) · `… MN2 datam2@` (`igpu.rs:2063`, n=1) · `… MODE active=` (`igpu.rs:2084`, n=1) · `… NOT-IN-IVB-PRM` (`igpu.rs:2086`, n=1) | **unflown** (new, GMUX8) | **it cannot fail and it cannot black the panel, and both statements are STRUCTURAL rather than promises: there is no `write_volatile` and no `dp_aux_transfer` call in the rung, every access is `mmio_read`.** It reads eight of the eleven registers G13 had to name as ABSENT — HTOTAL/HBLANK/HSYNC, VTOTAL/VBLANK/VSYNC and the link M/N pairs, all landed in `regs` from **IVB PRM Vol 3 Part 3 (North Display Engine Registers), Doc Ref `IHD-OS-V3 Pt 3 – 05 12`**, §4.1 pp.66–73 and §4.2 pp.73–77, tabulated in `gpu_spec.md` §6.3–§6.4 — plus `DP_CTL_A` (§4.4.1 pp.82–86) and `PIPE_CONF` (§5.1.3 pp.100–103), and prints **the mode the firmware is already driving**, which is the set of numbers the write rung has to reproduce. **WHICH PIPE IS READ, NOT ASSUMED:** `DP_CTL_A[30:29]` Pipe Select names it and the rung picks its timing block from that; `pipe_sel_raw=` is on the wire whatever it decodes to, and a Reserved `11b` falls back to pipe A under the visible name `A(pipe-sel-reserved)` rather than silently. **WHICH M/N SET IS LIVE IS ALSO READ:** `PIPE_CONF[20]` selects the normal (M/N **1**) or low-power (M/N **2**) DRRS set, both sets are printed in full, and `mn_set=` says which the `MODE` line used — a dry mode-set that silently read M1 while the panel ran on M2 would hand the write rung the wrong refresh with no way to notice. ⚠ **THE ONE DERIVED NUMBER, and both its inputs are cited:** refresh is not a register, so it comes from §4.2 p.74's own law `Link M/N = dot clock / ls_clk` with `ls_clk` from `DP_CTL[17:16]` (§4.4.1 p.85: `00b` 270 MHz, `01b` 162 MHz). **That is the first CITED link rate this ladder has ever had — and it does NOT retire G11's `rate_decode=TBV-tree-table`**, because the DPCD byte at `0x001` is a SINK capability in a DP-spec encoding while this is a SOURCE PLL setting in an Intel encoding. They are two different facts and the rung prints them side by side rather than letting one stand in for the other. A zero refresh is a REFUSAL and names its missing input (`undefined_why=link_n=0`, which is what an unpowered pipe will read) | G13 | **fixed-unflown** — and it closes G13's shopping list from the other end: `NOT-IN-IVB-PRM dp_tp_ctl dp_tp_status trans_ddi_func_ctl — population=IVB-PRM-V3Pt3(North)+V3Pt4(South), hits=0`. Those three occur **zero times** in either Ivy Bridge display volume: they are Haswell DDI registers, IVB trains eDP through `DP_CTL_A[9:8]` (§4.4.1 pp.85–86), and the eDP port is CPU-attached so no PCH transcoder is in its path. **R19 stated precisely: not *failed under: no citation*, but ABSENT FROM THE SILICON'S PUBLIC SPEC** — a stronger finding and a different one, which is why it rides its own token and not `NOT-IN-TREE` |
...
- **G6** — ~~one `awk` over the flight-5 capture~~ **READ, and FIXED; what would change the verdict
  ...
  1. `:: igpu-dpy: pre-switch state … SW_EXT=0x… SW_EXT_ST=0x… … gate=… ::` (`igpu.rs:1247`) — the
  ...
  2. Then either `:: igpu-dpy: rung=00 name=census ok=1 …` (`igpu.rs:1306`) — G6 ACCEPTS and G5
     opens — or the same `pre-switch-not-accepted` (`igpu.rs:1270`; the `why` token is deliberately
...
- **G7 / G5 / A7** — unreachable until G6 accepts, and GMUX-2 is the attempt to make it accept.
  ...
  (`igpu.rs:1278`) and `:: igpu: [GMUX] EXTERNAL restore=SKIPPED …` at the revert, and EXTERNAL does
...
- **G10 / G8 — the AUX timeout, and what the next boot must print.** Flight 10's rollup is
  ...
  1. `:: igpu-dpy: rung=03 name=pp ok=1 pp_write=DECLINED why=pp-bits-uncited pp_window=KEYED delays_programmed=0 pp_unwind=0 pp_ctl=0x… pp_sts=0x… on_delays=0x… off_delays=0x… div=0x… ::` (`igpu.rs:1462`)
     — the PPS frame **after** the mux moved, which no flight has ever printed; then
     `:: igpu-dpy: rung=03 name=pp SETTLE ms=210 budget=not-a-cited-T3 moved=<0|1> …` (`igpu.rs:1480`).
     `moved=1` would say the sequencer is live and sequencing on its own and re-opens the PPS half.
  2. `:: igpu-dpy: rung=04 name=dpcd try=<n>/7 ok=<0|1> …` (`igpu.rs:1512`/`:1530`), one line per
  ...
  3. The rollup's new tail: `… pp_seen=both pp=0x…/0x…->0x…/0x… pp_settle_ms=210
     aux_port=DPA(0x64010) aux_div=0x0C8 dpcd_tries=<n> ::` (`igpu.rs:1875`).
...
- **G11 / G12 / G13 — the three rungs GMUX7 built, and the ONE line that scores all three.** Flight 11
  ended `name=end ok=1` with the sink answering on the first try, i.e. the ladder ran out of rungs. The
  next boot's rollup tells you in one `awk` how far the new ones got, because `end` is now 09:
  1. `:: igpu-dpy: rung=06 name=link ok=1 window=DPCD(0x000-0x00F) dpcd_rev=0x… rate_raw=0x… rate_decode=TBV-tree-table lanes=… lanes_plausible=<0|1> …` (`igpu.rs:1633`)
     with its two companions, the raw `CAP 000:` dump (`igpu.rs:1635`) and `STATUS addr=DPCD(0x202-0x207) bits=uncited …` (`igpu.rs:1638`).
     `lanes_plausible=0` means `MAX_LANE_COUNT & 0x1F` fell outside `{1,2,4}` — the structural predicate
     `LADDER-igpu-bringup.md:326-327` states — and is PRINTED rather than made a refusal, because a capture
     that already cost a dark window should not be thrown away over a byte the next rung can carry.
  2. `:: igpu-dpy: rung=07 name=pps-on ok=1 pp_write=DECLINED why=pp-bits-uncited writes=0 … t_source=NONE t_dpcd=absent t_edid=absent …` (`igpu.rs:1686`).
     **This rung is expected to decline on every boot of this build.** A `t_source=` that is ever anything
     but `NONE` would mean the tree grew a citation, which is a code change, not a flight.
  3. `:: igpu-dpy: rung=08 name=link-train-dry …` (`igpu.rs:1713`) and its four companions. The line the next
     executor actually needs is `NOT-IN-TREE …` (`igpu.rs:1731`): it names every register a mode-set wants that
     this file has no offset for, `population=igpu.rs-regs-module(igpu.rs:5-93), hits=0`.
  **What would change G12's verdict is a DOCUMENT, not a boot**: `PCH_PP_CONTROL` (`0xC7204`) bit 0 and bit 3,
  and the `PP_ON_DELAYS` (`0xC7208`) / `PP_OFF_DELAYS` (`0xC720C`) T1..T5 field layout, from **Intel PRM Vol 3
  Part 4, "Panel Power Sequencing"**. Until that is in tree and cited in `gpu_spec.md`, the write half stays
  declined, on the wire, with its token — and G13's transcription stays a transcription.
...
needs to be scorable — **and, with no second knob, P7's post-enumeration second clear**. The full
wedge-theory ladder behind these rungs, and the field-by-field decode of every register named in
this section, are `PCIE-RP-RECOVERY.md` §11 (P7: §11.4).
...
| P6 BAR1WEDGE first-stall block | `UNAOS_BAR1WEDGE` | `:: BAR1WEDGE: rung=first-stall` (`pcihealth.rs:787`, n=1) · `[pcih] wedge-sample n=` (`pcihealth.rs:906`, n=1) · `[pcih] bar1wedge sticky-cleared at-arm` (`pcihealth.rs:866`, n=1 — the bare `sticky-cleared` prefix is n=2 in the source since P7 landed, so the token carries `at-arm`) · `[pcih] bar1wedge cto rp` (`pcihealth.rs:812`, n=2 — the armed and the UNREADABLE arm) | **never flown** — landed this arc, default OFF | — | P1 (the census resolves the root port and the ECAM page), P4 (it rides the same tripwire crossing) | **never-run** — the instrument that makes P5 scorable: it prints LNKCTL (which P4 reads and discards), the completion-timeout configuration (never read by anything, ever), and the deltas against an arm-time W1C clear of the three sticky latches |
| P7 SECSTA2 post-enum clear (**CONTRACT AMENDED — WIFISWEEP**) | `UNAOS_BAR1WEDGE` (**no new knob** — the same feature as P6) | `[pcih] bar1wedge sticky-cleared post-enum … site=post-wifi walks_since_enum=<n> walkers=<names>` (`pcihealth.rs:1175`, n=1) · its no-device twin `[pcih] bar1wedge post-enum clear REFUSED reason=no-root-port … site=post-wifi` (`pcihealth.rs:1110`, n=1 — **new**: the silent return is gone, so the ORDER is scorable on q35) · the call sites, `note_wifi_census()` + `sticky_clear_post_enum()` at `main.rs:1261`, `main.rs:1745`, `main.rs:6013` (line-neutral appends after `wifi::service()`, the three storage-ready passes; **MOVED out of `arch/x86_64/pci.rs:1148`, which keeps its line and a statement-free comment**) | **never flown** — landed this arc, default OFF | — | P6 (it re-baselines P6's statics and prints beside P6's lines), P1 (`PCIH_READY`, which only `census` sets) | **never-run** — the second W1C clear, taken after the LAST config-space walk of the BOOT, so P6's `d_secsta`/`d_lnksta`/`d_devsta` at the first stall are deltas against the end of ALL walks instead of against `pci::init`'s Kepler dispatch. Its `relatch=` field is the finding in its own right: which RW1C bits the boot's walks set on this bridge — the premise `PCIE-RP-RECOVERY.md` §1.3's whole "residue" reading rests on and which nothing had ever measured. **WHAT THE AMENDMENT CHANGED (ledger B113, `PCIE-RP-RECOVERY.md` §11.5):** as landed, "the last bus walk" meant the last walk of `pci::init`, and flights 8/9 showed that is not the last walk of the boot — `wifi::bus::census` (`wifi/bus.rs:125`) sweeps buses 0..=255 from the main loop 253 ms later and both flight images carried `UNAOS_WIFI=1`, so the stored baseline predated the boot's last master-abort source. The rung is now one-shot, its call site is the main loop BELOW that sweep, and the line names what ran between enumeration and the clear so `relatch=` is attributable. Building the ORIGINAL rung had already corrected §11's named residual once: the EHCI `0..=255` walk (`arch/x86_64/pci.rs:838`) runs BEFORE the at-arm clear (`arch/x86_64/pci.rs:1016`), so it was never the leak |
...
- **P7** — it flies on the same boot as P6, costs nothing extra to arm, and settles one thing on its
  own: `relatch=secsta:2000` measures that a bus walk on this bridge latches Received Master Abort
  and `relatch=secsta:0000` measures that it does not. Both readings are new; the second is the one
  that would make a `d_secsta=2000` at the first stall hard to explain away. Score card:
  `PCIE-RP-RECOVERY.md` §11.3, the `relatch` rows.
...
| KF9 `CTRL_ADDR TARGET` | `UNAOS_KEPLER_CTRL_ADDR` (`nvidia-kepler-ctrladdr`) | `3620c7d5`, deleted by `51b98bab` ("kepler-fence pull 15") | `kepler.rs`, pre-init | **RESTORED IN FULL** (2026-09-17) — the 3×4 reversible TARGET audit came back 2026-09-15; the s13 original's surrounding apparatus, the whole channel bringup re-run *inside* the target loop, is back now as rung **KF9b** behind its own default-OFF knob `UNAOS_KEPLER_CTRLBIND` (`nvidia-kepler-ctrlbind`, implies this row's feature). The bringup no longer "no longer exists in this shape": it was CARVED OUT of `kepler::init` into `kepler::try_bind_and_witness`, which `init` still calls once, `#[inline(always)]`, exactly where that block sat. ⚠ The carve-out MOVES THE ARMED IMAGE — 414 bytes inside `kepler::init` — and leaves the knob-off image byte-identical (both `knoboff` verdicts exit 0); the numbers and what was ruled out are in the seam's own comment and the CTRLBIND commit body. rmbp-queue `SHUTRESTORE-CTRLADDR-BRINGUP` is closed as DONE (code); the flight is owed — **AND NOW RESTORED IN FULL AND FLOWN** (flight 11, 2026-09-22): the restored apparatus ran on metal, all twelve encodings WRITABLE so none was skipped, twelve `witness=silent` rollups, and §2's KF9 row closes as **shut-out by measurement**. R19's round trip is complete for this rung — code deleted, code restored, apparatus restored, flown, and the reading it bought is one the deletion had made unobtainable. The flight also returned a restore defect (`verdict=DIRTY`, `chan04` off in bit 0) which is fixed in the same file by KF9CLOSE (2026-09-22, ledger B133); see §2's KF9b row for the mechanism. `docs/dev/OS/rmbp-queue.md` `SHUTRESTORE-CTRLADDR-BRINGUP` is now DONE on both halves, code and metal |
...
state DDC=… SW_DISP=… SW_EXT=… DISP=… EXT=…` at `igpu.rs:1334` **before** it refuses at `:1357`, so
...
| §2 Kepler FIFO / PBDMA | 30 | 6 | 11 | 2 | 11 |
...
anywhere in this file is "ruled out."** ⚠ **THE §2 ROW WAS RE-COUNTED FROM §2'S OWN
STATUS CELLS on 2026-09-22 (KF9CLOSE) and was stale by three rungs** — it read `27 / 5 / 10 / 1 / 11`
while §2 listed thirty: KF9b and KF29 had been added and KF27 had moved to `open` without the table
following. KF9CLOSE's own change is one rung, KF9b `never-run` → `shut-out`; the other two are
arithmetic that was already owed. **No other section's row was re-counted, so §1 and §3–§6 are as
found and may carry the same debt.**
...
flight lines in [`KEPLER-METAL-LOG.md`](KEPLER-METAL-LOG.md). ONE carve-out stays open as a
`docs/dev/OS/rmbp-queue.md` row: the two knobless hard-codes `run_recon` / `do_takeover` still
cannot be re-armed from a media build (`SHUTRESTORE-KNOBLESS`). The other, KF9's per-TARGET
channel-bringup re-run, is CLOSED — the seam is carved out of `kepler::init` as
`kepler::try_bind_and_witness` and the rung is §2's KF9b behind `UNAOS_KEPLER_CTRLBIND` (CTRLBIND,
2026-09-17) — **and that flight is flown: flight 11, 2026-09-22, twelve binds and twelve silent
witnesses, so KF9 owes NOTHING further and leaves this file's owed-flight list.** What it does not
leave is the twenty-one-rung R19 list above, and the distinction is the honest one: flight 11 ran
on the same never-ran-FECS host as s13, so the condition that keeps KF6/KF8/KF9 on that list was
not varied by it. The flight also bought a restore defect and its fix (§2 KF9b, ledger B133).
