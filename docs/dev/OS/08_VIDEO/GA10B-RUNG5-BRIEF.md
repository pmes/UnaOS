...
*(orin-0912b, 2026-09-13, GA10B6. Ledger A72 (the second triple), A73 (the mapping), A74 (the
...
**The order is A72 first**, on R53 Q7's own terms — "if acr-gsp is refused the safety-scheduler triple
is the next boot", and it was refused. A73 follows on the boot after. Each arm is built, knobbed and
...
signature blob and of nothing else in the directory, which is why A73 does NOT permute the manifest
...
read**. So the A72 card is the A63 card with three more files copied in from the staged set (they are
...
The A73 boot is the same wire with `UNAOS_GA10B_PROBE5=4`, `triple=acr-gsp map=swapped`, and the
...
### 8.7 The announce-before-write counter was lying (A74)
