...
`:: DESK: desktop armed ::` lands at log line **1496** — 381 lines *after* the render task was
...
| **VUGART** (this arc, rmbp-ledger **B162**) | Peter, flight 11 2026-09-22: *"vug opens struggling so it tries to display the basic line drawing but it turns into abstract art."* The crystal is ALWAYS a line drawing, so "abstract art" means the lit pixels on ONE surface are not ONE wireframe. **The defect was the RELEASE WORD**: `PHASE.store(frame + 1)` derived the workers' release edge from the PRESENTED-frame counter, which VUGTRUTH (row above) had deliberately frozen on a failed present — so the next frame republished a value the workers already held, no edge, no release, `DONE` never reached `live`, and the parent blocked at the barrier having ALREADY published the new projection and rasterised its OWN band `BAND_PAR..SH`. The surface it leaves holds rows 0..216 at one rotation and 216..288 at the next, and the compositor composites a live window from the SURFACE on the panel's cadence, not on this program's presents. Fix: `PHASE.store(gen)`, `gen = attempts + 1` — `attempts` advances on every RENDERED frame whatever the panel did (REVIEW D5's counter, for D5's reason). Plus the instrument that makes it countable: per-band `GEN`/`BUSY` stamps scored before every present AND on the barrier's park pass, emitting `:: VUGART: frames=<n> coherent=<n> torn_rows=<n> mixed_frames=<n> -> PASS\|FAIL ::`. | **The FIX is arch-neutral** (one token, shared code). **The INSTRUMENT is x86-only, and that is a `.text` measurement, not a preference**: the aarch64 image's hard ceiling is 0x2000 and its `.text` ends at 0x1f53 — **173 bytes** — while the x86 image is capped at 0x3000 and ends at 0x212f. Every hook is an empty `#[inline(always)]` fn on aarch64 and `BUF_CAP` is `cfg`'d, so the stripped aarch64 `VUG.ELF` is BYTE-IDENTICAL across the instrument commits (sha256 `1adbec39…`) and the 300-frame checksum `0xe68285b85121ac7c` cannot have moved. | **LANDED.** Pinned in `scripts/specs/x86-wc.spec`; go-red measured (`PHASE.store(1)` -> `frames=2 coherent=1 torn_rows=216 mixed_frames=1 -> FAIL`, and WINX-8 red beside it at `presents=1`). **OWED to the Pi:** the aarch64 image has no instrument, so a Pi vug's coherence is unscored — it needs `.text` headroom first. |
...
RECONCILED 2026-08-19 (pi trunk landing): the census body above (§1–§7) is the `hw-pi4`
document; the Orin section below arrived as the `hw-jetson` add/add fragment and is appended
here as §8, exactly as that fragment's integrator note directed.
