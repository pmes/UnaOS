...
· NEW  GMUX-2  fix the pre-switch gate's EXTERNAL term, and it is ONE decision with TWO halves — GMUX-1 proved the naive half is a panel hazard. Gate on the STATUS ports only (`DDC`, `READ_DISPLAY` 0x11, `READ_EXTERNAL` 0x41) and drop `SWITCH_EXTERNAL` (0x40) to printed-not-gated, as `SW_DISP` already is. But `p_ext` is ALSO the restore value: `igpu.rs:1289` and `:1309` push `unwind.push_gmux(GMUX_SWITCH_EXTERNAL, p_ext as u8)`, so dropping the gate term alone writes `0x01` — a value no encoding in the file names — into the external display mux on every exit path, the silent state change `igpu.rs:270-272` forbids. Restore the member `READ_EXTERNAL` validated instead. No boot to design; a boot to score. Unblocks G5, G7, G9, A7. Register §5, finding `docs/dev/evidence/rmbp-0915/GMUX-1-PRESWITCH.md`
