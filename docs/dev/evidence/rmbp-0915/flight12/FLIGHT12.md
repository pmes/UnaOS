...
- Trackpad: DEAD — `:: EHCI-HID: [1] STOP-NOTE interrupt endpoint halted addr=6 ep=IN1 kind=boot-mouse mps=4 class=xact-err-burn tok=0x00048141 cerr=0 halted=1 … reports=0` at 32422 ms (`drivers/ehci/mod.rs:13863`; the mod.rs:14457 table names token 0x00048141 as the 05ac:820b BT-HID-proxy mouse). `[deadman] … hid=0 … in=0/0/0` from then on. The ONE `[cursor] armed x=961 y=617` at 50266 ms follows `[ptrdead] backlog … pushed=192 … travel=(192,-192)` — PTRDEAD's synthetic motion, B186's Class 3 leak, on metal.
  - **CORRECTED (MOUSEHALT M1, rmbp-ledger B190; the wire beats this record).** addr=6 is not the trackpad. It is `05ac:820b` (`M1 hub-downstream device addr=6 05ac:820b … (parent hub 4 port 2)` at 1775 ms), the Bluetooth HID proxy on the Broadcom hub, and it retires with this token on flights 8, 9 and 11 too. The trackpad is `addr=8 ep=IN1 kind=vendor-mt`, and it never halted. "`hid=0` from then on" is false: `[deadman] … hid=61/116/128/68/63` at 235–239 s are 436 pad completions (`vendor-multitouch raw report #2 (58 B): 74 57 1c 03 …` at 234250 ms). They followed the first-ever `[tp] mode … try=legacy-index0 … latched=yes`, and `trackpad_dispatch` routes a first byte of 0x74 nowhere, so zero pointer events came from the pad. `in=0/0/0` is the channel's normal reading, not an absence of input. Record: `docs/dev/evidence/rmbp-0915/mousehalt/MOUSEHALT.md`.
