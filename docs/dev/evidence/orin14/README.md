...
| `NET4A.md` | gap #2's first metal question built: the RTL8168 RX ring + 32 buffers placed below 4 GiB (no inbound-iATU alias), the `[net4F] distinct buffers-written(count=N)` witness, the scorer for the next `UNAOS_NET4=1` flight, and why NET-4o's boot-19 "no clean low span" was never a measurement | executor NET4A |
