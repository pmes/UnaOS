...
| 2 | next | With the domain ON and the DTB clocks ENABLED (via BPMP), does PMC_BOOT_0 answer with an Ampere chip id — and which clocks did UEFI leave running? | BPMP MRQs only (PG SET_STATE, CLK ENABLE/DISABLE), symmetric | BAR0 + 0x0 (PMC) | **FLOWN — PASS** 2026-09-06 (`ga10bprobe2`): `boot0=0xb7b000a1` → chipset `0x17b`, arch `0x17` Ampere, impl `0xb`, rev `0xa1`; clocks `304 41 236`, all three OFF as found, 236 refused with `err=-22`. [`../orin16/PROBES-2026-09-06.md`](../orin16/PROBES-2026-09-06.md) §4, ledger A24 |
| 3 | +1 | What did the platform firmware leave in the GSP boot-config (BCR) registers, the PMU falcon, the security fuses and MC enables? Is any secure image staged? (+ the rung-2 leftover: what IS DTB clock id 236, and why did BPMP answer `err=-22`?) | none (BPMP MRQs only, all pure queries) | GSP priscv BCR block, PMU falcon2, fuse (3 more), MC | **fixed-unflown** — built as `ga10bprobe3` (`UNAOS_GA10B_PROBE3=1`); as-built spec [`../orin16/GA10B-RUNG3.md`](../orin16/GA10B-RUNG3.md), ledger A39 |
| 3b | +1 | Does the GSP falcon come out of an engine reset halted, and does a Falcon MAILBOX scratch write read back? | **first GA10B MMIO writes**: pgsp_falcon_engine reset assert/deassert; one MAILBOX write | none new (GSP falcon) | **fixed-unflown** — built as `ga10bprobe3b` (`UNAOS_GA10B_PROBE3=2`), the first write rung; [`../orin16/GA10B-RUNG3.md`](../orin16/GA10B-RUNG3.md) §4 |
