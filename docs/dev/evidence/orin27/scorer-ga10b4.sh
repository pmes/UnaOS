...
#   usage: scorer-ga10b4.sh <log> [4a|4b|4c|4e|4f|5a]  exit 0 = the rung's PASS arm present and STOP-check clean
...
# orin-0912b (2026-09-13, ledger A82): THE ANNOUNCE/RESULT COUNTERS WERE MISCOUNTING, on both sides.
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  # ── REALF: the real capture as a POST-FIX build prints it (orin-0912b 2026-09-13, ledger A82) ────────
  ...
  # ── ANNOUNCE-BEFORE-WRITE, the counter's own go-red proof (ledger A82) ─────────────────────────────
...
# IN-FAMILY count (orin-0912b 2026-09-13, ledger A82). The counters below were UNBOUNDED: `cnt " wrote=0x"`
...
case "$RUNG" in
  ...
  5a)
    ...
    # ledger A80/A81 on, and a capture from before them carries neither — so this says what it sees and
    ...
    off=$(cnt "PSCI SYSTEM_OFF (0x84000008) via SMC")
    echo "5a verdict lines = $vl (must be exactly 1); ACR-ACCEPTED = $acc (must be 1); ${form:-form=? shift=?}; ${tm:-triple=? map=? (a capture from before ledger A80/A81)}; SYSTEM_OFF lines = $off (must be >= 1)"
