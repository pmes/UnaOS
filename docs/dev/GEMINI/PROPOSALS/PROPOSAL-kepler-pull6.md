STATUS: CHANGES REQUESTED (2026-07-22, reviewer). Derivation 2 (SUBFIFO_ENG_MASK[0]=1 at 0x2390 to bind PBDMA→PGRAPH; pbdma-count 3=0b011=2 units) APPROVED to try, guarded. §3 cleanroom (undocumented-confirmed → honest empirically-probed note + bad-read guard) APPROVED. DERIVATION 1 REJECTED AS CIRCULAR: the proposed head address (NV_PDISPLAY_BASE + 0x400 + head*0x300 + 0x60) is BYTE-IDENTICAL to the pull-4 address that already read uniform ZEROS in sitting #3 — the guard will just re-confirm zeros. The HEAD_CAP analysis (explaining pull-5 statics) is correct and excellent; but before reusing a known-zero address, answer: is the EVO ARMED shadow only populated AFTER the core channel is activated, and does our read happen before that? OR — the real possibility to name — if firmware posted the display via VBIOS/CRTC direct writes rather than the EVO core channel, the EVO armed shadow is legitimately zero and scanout must be read from the ISO/DMA hub or direct CRTC regs instead. Pick a genuinely NEW candidate source with that reasoning, not the zero address again.
