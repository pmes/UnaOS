...
|---|---|---|---|
| 31 | PROPOSAL-kepler-fence-pull31 | LANDED 708041ae; all 3 amendments honored (readback-per-write w/ FAULT-skip; post-bind witness leg byte-identical to the original write; ENGINE_STATUS raw pre+post); gated green; awaiting s35 | FIRST CONTEXT-BIND WRITE: inst_off>>12 → CHAN_CUR/CHAN_NEXT; does PFIFO's strip move for the first time? |
| 30 | PROPOSAL-kepler-fence-pull30 (retroactive backfill, accepted; PBUS_INTR decode delivered: bit2 MMIO_RING_ERR + bit3 MMIO_FAULT) | LANDED 0e26447e; matches invitation exactly (safest-first chain 0x804/0xb00/0xb04/0xc00/0xc08, taint-skip, PRING clear + un-wedge test at first fault, 0x409504 excluded); s34: ⭐ **ALL FIVE CLEAN — 0x409504 CONVICTED by elimination**; six of seven offsets exist, =0 at rest; context surface (CHAN_CUR/ENGINE_STATUS) reachable and empty — consistent with err=2 = no context bound; un-wedge still unexercised → pull 31 (proposal-first); PBUS_INTR decode still owed | chain-read five unknown offsets; first FAULT triggers the real un-wedge experiment |
| 29 | PROPOSAL-kepler-fence-pull29 | LANDED bbef2747; s33boot1: ⭐ **GATING THEORY REFUTED both ways** (enable bit already SET; CC_SCRATCH[0] first-read = real 0) — poison is PER-OFFSET (0x409504 the suspect); CC_SCRATCH[0] exists =0; un-wedge untested (nothing wedged); PBUS_INTR=0C latched, W1C'd → pull 30 | CTXCTL subunit-gating theory + PRING fault observe/clear + rotation to 0x409800 — why 0x409504 faults and how to un-wedge |
| 28 | BRIEF-kepler-fence-pull28-fecs-ctx-study / PROPOSAL-kepler-fence-pull28 / STUDY-fecs-ctx-init | LANDED e23fdcbd (proposal+study+probe in one); approved w/ binding amendment (BADF1000 = offset absent; no H-writes vs unverified offsets; H2 needs writability evidence); s31: ⚠ **PRI-POISON LAW found** — first bad 0x409xxx read (0x409504) BADF1000-poisons the whole FECS unit for the boot; only first datum clean, six confounded; ucode/hb verify-gates aborted cleanly; recon RELOCATED post-hb-final + control-bracketed (GR5 inline); s32: **CLOSED — poison law double-confirmed by control frame (recon-pre real / recon-post BADF1000), s30 fully restored, 0x409504 = the one clean datum; six offsets still confounded → pull 29** | cleanroom STUDY of FECS context/init ucode + read-only CTXCTL recon — what does REQUIRE_CTX guard? |
...
**Coordinator arc CONSOLE-ON-PANEL (b5918496): ⭐⭐ METAL-CONFIRMED s33boot2 — Peter: console "prints text very well". THE DISPLAY LANE GRADUATES (measurement → mapping → ownership → console).** Follow-on routed: SMC-BATT 1/s witness scrolls forever on the now-user-visible console — quiet-boot-gate it (SMC lane). Root cause: fbcon never drew on the metal media — `_print` QUIET-PANEL-gated off without `bootlog`, `milestone` cfg'd out under `usbdebug`; geometry was always right. Fix: `PANEL_CONSOLE` override + scale-6 glyphs + `panel_console_resume()` seam after fb-draw done. Markers: `fbcon: glyphs-active …`, `kdisp: console-repaint rows=N`. Arc note: `Kepler/ARC-console-on-panel.md`. (Sibling arc SERIAL-ON-GUI: executor tripwired correctly — premise false, FTDI mirror is unconditional and works on GUI features in QEMU; metal fork test = run `bootlog` verb at a GUI sitting, check `ftdi:console-up` tag. No code.)

## Lane 2: Kepler DISPLAY (scanout — kepler_display.rs after split) — `video/Kepler/`, files `*kepler-display-pull<N>*`
| Pull | Files | Status | One-liner |
|---|---|---|---|
| 20 | BRIEF-kepler-display-pull20-fbcon-on-panel | LANDED e7db05e4 (+inline fix: base via `current_base()`); s30: **HYPOTHESIS REFUTED — GOP reports 4096-px stride, match=true; fbcon is NOT mis-strided**; console failure is elsewhere; glyph probe under-sized (8×8 px ≈ 0.7 mm — nothing visible, execution confirmed on serial) | reconcile fbcon stride vs hw 16384 and get console-shaped output on the panel |
