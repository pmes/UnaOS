...
|---|---|---|---|
| 28 | BRIEF-kepler-fence-pull28-fecs-ctx-study / PROPOSAL-kepler-fence-pull28 / STUDY-fecs-ctx-init | LANDED e23fdcbd (proposal+study+probe in one); approved w/ binding amendment (BADF1000 = offset absent; no H-writes vs unverified offsets; H2 needs writability evidence); s31: ⚠ **PRI-POISON LAW found** — first bad 0x409xxx read (0x409504) BADF1000-poisons the whole FECS unit for the boot; only first datum clean, six confounded; ucode/hb verify-gates aborted cleanly; recon RELOCATED post-hb-final + control-bracketed (GR5 inline); s32 pending | cleanroom STUDY of FECS context/init ucode + read-only CTXCTL recon — what does REQUIRE_CTX guard? |
