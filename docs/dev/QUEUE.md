...
· NEW 2026-09-12T2x:xxZ (SERDRAIN, executor finding): the x86 S5 route is only HALF drained — `power::shutdown` now drains the staging ring before the SMC/ACPI call (SO31 part 3), but `video/crystal.rs` Shut Down and `video/instgui.rs` call `arch::acpi_power::poweroff()` DIRECTLY and reach S5 with the ring unflushed. One statement at the top of `poweroff()` in arch/x86_64/acpi_power.rs. Owner: the next x86 power/serial arc. `open`.
· NEW 2026-09-12T2x:xxZ (SERDRAIN, executor finding, measured): the three serial-ring fixtures (`draincap_selftest`, `backpressure_selftest`, `pwrdrain_selftest`, serial_ring.rs mirror_service) each fill the ring to 64 lines once per boot on EVERY image — none is `witness`-gated, so esp-jetson (witness-free by design) still carries 12,928 B ≈ 1.1 s of UART per boot at the first mirror_service. SO30 was about a witness eating UART budget. Put them under the `witness` feature like every other fixture (knob-off byte identity measured). Owner: the next serial arc. `open`.
· NEW 2026-09-12T2x:xxZ (GA10B4E, executor finding, measured): `./arroyo knoboff` hardcodes ONE shared scratch directory (`~/unaos-bench/scratch/knoboff`: one worktree, one CARGO_TARGET_DIR, one set of `flat-*.bin`), so two concurrent invocations corrupt each other's verdict — six executors ran it between 211452Z and 212300Z and one got exit 1 "x86 image MOVED, SIZE changed" for an aarch64-only delta; alone it exits 0. A gate that can report MOVED for someone else's build teaches re-run-until-green. Fix: a per-invocation directory (pid or sha-keyed) or a lock with a named waiter. Owner: whoever next edits arroyo (pair with the HEAD~1 row above). `open`.
· NEW 2026-09-12T2x:xxZ (NET6, executor finding, measured): `./arroyo knoboff <feature>` defaults its baseline to `HEAD~1` (arroyo:8435), which on a MERGE commit is the FIRST parent — an executor that merges its track in and re-gates compares its tree against its OWN pre-merge tip, across every track commit, and gets exit 1 "SIZE changed, this is code": true, and the wrong question. The tree-without-the-arc on a merge is the OTHER parent. Measured on exec-orin-net6: plain `knoboff net4` exit 1 (x86 1,210,312 bytes differ); `knoboff net4 <second-parent>` exit 0, control fired, identical current-tree hashes. Fix: default to `HEAD^2` when HEAD has two parents, or refuse with NO VERDICT (exit 2) on a merge HEAD and print both parents. Until then every merge-tree knoboff names its baseline. Owner: whoever next edits arroyo. `open`.
