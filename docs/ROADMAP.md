...
| **LOGIN** | A human user on UnaOS (RULINGS R51): `fs/users.rs` (`USERS.DAT` record store — name, salted SHA-256 credential, home path), the `user:<name>` SESSION PRINCIPAL stamped into the loader's own per-slot principal on both arches and enforced by the existing `SYS_OPEN` owner/grants check, `login`/`logout`, the M1 fixture. **M1–M4 fixed-unflown on `exec-orin-login`** (2026-09-12: `/home/<user>` at first login on the EL0 FAT volume, the knob-on aarch64 path-taking `sys_open`, the home ACL proof on both arches, the login screen at desktop boot with the keyboard taken ahead of every serial echo, and **Log Out as a row in the crystal menu** — the session drops and the screen returns, where a second login opens a new session; fixtures behind `UNAOS_LOGINST=1`). THE ARC IS COMPLETE and unflown: the next Orin render boots to the login screen with `UNAOS_LOGIN=1` added to the flight line. Blocked-on-record: one `HOST_VERBS` row (`libs/sys/midden_core`), one builder map line (`builder/src/main.rs`); LEDGER SO32 | jetson (shared code, both arches) |
...
surface. The socket syscall family opened at number 19 (SOCK-2 landed 19–22, SOCK-3 landed 23–25; SOCK-4 added no syscall — a socket rides the existing `SYS_XFER`/`SYS_RECV`). **SOCKNUM (WINX-1, 2026-07-29): the family now lives at 40–48** — 19–27 collided with aarch64's `MSEND`/`MRECV`/`THREAD_*`/`FB_*`/`FUTEX`/`INPUT_POLL`, violating the cross-arch shared-number law (a number names the same verb on every arch). Bringing the x86 WINDOW verbs up made the collision load-bearing, so the x86-only socket family moved, relative order preserved. Next free number: **49**. See [`08_NET/networking.md`](../unaos/docs/dev/OS/08_NET/networking.md) § SOCKNUM.
...
| SOCK-2 ✅ 🔬 | **Landed (round 9, `net-sock1`).** The UDP socket syscall family over a persistent smoltcp `SocketSet`, behind `UNAOS_SMOLNET` (knob-off byte-identical, both arches): `sys_socket`(40)/`sys_bind`(41)/`sys_sendto`(42)/`sys_recvfrom`(43). A socket is a capability (`KIND_SOCKET`, send=`CAP_WRITE`/recv=`CAP_READ` at the File `handle_resolve` CHECK); `recvfrom` is non-blocking (`-EAGAIN`). A ring-3 fixture completes a datagram round-trip to slirp's DNS (`:: SOCK-2: ring-3 udp round-trip … -> PASS ::`) + a kernel witness (`:: SOCK-2: … — witness OK ::`). Next free syscall number: **23**. See [`08_NET/networking.md`](../unaos/docs/dev/OS/08_NET/networking.md) + [`SECURITY.md`](SECURITY.md). | x86 |
| SOCK-3 ✅ 🔬 | **Landed (round 10, `net-sock1`).** TCP **client** sockets over the same persistent smoltcp stack, behind `UNAOS_SMOLNET` (knob-off byte-identical, both arches): `sys_connect`(44)/`sys_send`(45)/`sys_sock_recv`(46); `sys_socket` gains `SOCK_STREAM`. A TCP socket is the same `KIND_SOCKET` capability (connect/send=`CAP_WRITE`, recv=`CAP_READ`); `connect` is non-blocking with a ring-3 poll model (`0`/`-EINPROGRESS`/`-ECONNREFUSED`), `recv` non-blocking (`-EAGAIN`/`0`-EOF). Carries the two SOCK-2-review REQUIRED folds: the socket handle value word is now **gen-fenced** `(gen<<32)|(sid+1)` (recycled-slot UAF closed for all socket kinds), and every TCP pump releases the `STACK` lock between chunks. A ring-3 fixture completes a byte-stream round-trip to slirp's resolver over DNS-over-TCP (`:: SOCK-3: ring-3 tcp round-trip … -> PASS ::`) + a kernel witness (`:: SOCK-3: … — witness OK ::`). Next free syscall number: **26**. See [`08_NET/networking.md`](../unaos/docs/dev/OS/08_NET/networking.md) + [`SECURITY.md`](SECURITY.md). | x86 |
| SOCK-4 ✅ 🔬 | **Landed (round 13, `net-sock1`).** Scope B — **transferable sockets**: a socket capability moves across processes (the U7x/U8x console-cap transfer, socket edition), behind `UNAOS_SMOLNET` (knob-off byte-identical, both arches), with **no new syscall** (a socket rides the existing `SYS_XFER`/`SYS_RECV`). `sys_socket` now mints `CAP_READ|CAP_WRITE|CAP_GRANT` (so the owner may delegate its own socket; still attenuable to send-/recv-only), and `sys_recv` MIGRATES the socket's registry ownership to the grantee (`xfer_socket_migrate` → `smolnet::reassign_owner`, gen-checked) so the moved cap resolves under the new row. Single-owner throughout: the grantor's migrated-away handle is `-EACCES`, and the SOCK-3 gen fence rejects any stale cross-row handle to a freed+reused slot (no rebind) — discharging the SOCK-2 review's transferable-socket UAF warning by construction. A two-fixture ring-3 demo (`sock4-grantor`/`sock4-grantee`) round-trips a UDP socket on the MOVED cap + a kernel-side gen-rebind proof (`:: SOCK-4: transferable sockets … -> PASS ::`). See [`08_NET/networking.md`](../unaos/docs/dev/OS/08_NET/networking.md) + [`SECURITY.md`](SECURITY.md). | x86 |
| SOCK-5 ✅ 🔬 | **Landed (round 14, `net-sock1`).** Scope B — **DHCP via smoltcp**: the persistent stack acquires its own IPv4 lease with a `dhcpv4::Socket` instead of copying the static address the hand-rolled DHCP set, behind `UNAOS_SMOLNET` (knob-off byte-identical, both arches), with **no new syscall** and **no new ring-3 surface** (DHCP is kernel-internal interface config). `ensure_stack` builds the interface address-less + adds a kernel-internal DHCP socket (a reserved `NSOCK+1`-th storage slot, never in `reg`); `configure_via_dhcp` pumps a bounded loop to `Event::Configured` and applies the leased address + router, falling back to the static `hw_addr` lease on a silent server so SOCK-1/2/3/4 never regress. A kernel witness (`:: SOCK-5: smoltcp dhcpv4 lease 10.0.2.20/24 gw 10.0.2.2 — witness OK ::`) proves the interface now uses a DHCP-obtained address (`.20`, not the `.15` static default) and the SOCK-2/3/4 round-trips all still pass on it. See [`08_NET/networking.md`](../unaos/docs/dev/OS/08_NET/networking.md) + [`SECURITY.md`](SECURITY.md). | x86 |
...
beat it (details and caveats in [`unaos/libs/fs/unafs/README.md`](../unaos/libs/fs/unafs/README.md)):
...
| K4 | **✅ Journaled kernel writes (landed QEMU-green 2026-07-14; metal write→power-cycle→boot-2 byte-verify rides the next attended Pi bench).** `fs/unafs.rs`'s `write_sector` now routes to the hardened block layer (`drivers::block::write_block`, emmc2 CMD24 + R1/CMD13), so the mount is read-WRITE. Coherence keystone: a single, process-wide, IRQ-masked mount (`with_unafs`, modelled on the F3 `NAMESPACE` lock) — one authoritative in-RAM allocation bitmap + journal cursor, every access serialized; per-call mounts would diverge two bitmaps (corruption). Keeping one mount live also means a pure read (`uls`/`ucat`) never fires the crate's `Drop`-time metadata write-back. Shell write verbs `utouch`/`uwrite`/`umkdir`/`urm` (write-through, durable) + a byte-verified `K4-write` witness (`w=0x7f`: create+write → remount → byte-verify → delete → remount → negative → clean journal). **Honest torn-write scope (`SECURITY.md` §K4):** the WAL is intent-logging only (dirty detection, no replay/rollback); crash safety is by write ORDERING (leak-not-dangle), and the crate's "single-block atomic swap" is truly atomic only when the record fits one 512 B sector (a 4096 B block is 8 non-atomic sector writes). A minimal VFS (FAT + UnaFS coexistence) is deferred |
| K8a | **✅✅ Copy-on-write core (landed + METAL-CONFIRMED 2026-07-16 same-day attended sitting: pristine-v3 boot 43/43 AND a real v2→v3 migrated-volume boot 43/43, 0 forbidden both — migration byte-verified on silicon).** Format v3: every mutation allocates fresh blocks (never overwrite in place); each public op is ONE transaction committed by an atomic 512 B root-sector flip — A/B generation-stamped, checksummed root slots in block 1 (mount picks the newer valid slot), so a power cut anywhere — including a tear inside the root write itself — yields the old tree or the new tree, never a hybrid. Inode ids became STABLE LOGICAL numbers resolved through a CoW'd on-disk inode map (Fork-1 verdict); the allocator became a refcount map (free ⇔ reachable from no retained root; two-view current/frozen discipline enforces never-overwrite); the WAL is GONE (atomicity is structural — F1 superseded). The format speaks the FUTURE shape day one: the root reaches a snapshot index and a persistent reclaim queue (both ordinary UnaFS objects; v1 policy: cap 16, eager drain — a pending queue found at mount is drained crash-safely in one commit). New golden-vector KATs pin v3 (incl. the root-fits-one-sector KAT + compile-time assert); the kept record encodings (Inode/Extent/AttributeValue/DirEntry/CatalogEntry) retain their ORIGINAL goldens. One-way migration: `tools/unafs migrate` walks a pre-K8 (v2) volume read-only (`unafs::legacy`) and replays it into a fresh v3 image (do-it-right: no runtime compat with our own old format). Kernel: `with_unafs`/K4 verbs/K6 ACL/BANDY all ride the new path unchanged (43/43 MBENCH); new uncounted `K8a-cow` witness proves gen-advance, power-cut-mid-commit convergence to the OLD tree, refcount persistence across remount, and live commit-path bench counters (CNTPCT ticks + blocks written per commit — the vaire ruling). SECURITY.md §K4 torn-write ledger: ✅ FORMALLY RETIRED (metal verdict 2026-07-16) |
| K8b | **✅✅ Retained roots (snapshots) + reclamation (landed + METAL-CONFIRMED 2026-07-17, single attended boot: MBENCH 44/44 on sitting).** `snapshot_create` retains the committed tree as a generation-stamped entry (name/creator/timestamp K6 typed attrs) in the on-disk snapshot index and increfs EVERY block the retained root reaches — the security core: free ⇔ reachable from no retained root, so no live mutation can ever reallocate a block a snapshot still lives on (the two-view current/frozen discipline extended; never-overwrite + root-flip atomicity preserved). v1 policy cap 16 (refused cleanly, no format change). `snapshot_drop` never frees directly (design V2): it removes the index entry AND enqueues the snapshot's blocks in ONE atomic commit, then eagerly drains (decref — a block survives iff another live/retained root still reaches it); a power cut mid-drain resumes from the queue on the next mount. fsck's reachability walk now spans every retained root and repair rebuilds TRUE multi-root refcounts. User-visible day one (Peter's V3): shell verbs `usnap`/`usnaps`/`usnapdrop` + host `unafs snap`/`snaps`/`snapdrop` (owner-or-kernel drop authority, the BANDY ruling). Bench counters: snapshots created/dropped (vaire ruling). New uncounted `K8b-snap` witness: snapshot→overwrite→snapshot reads OLD bytes→retention-safe alloc→drop reclaims→power-cut-mid-drain converges, self-cleaning |
| K8c | The two-root diff walk (descend only where child pointers differ) + the vaire/Bolt integration surface |

Order: F2 → K1/K2 → **K3 (metal milestone)** → K4 → **K8a (CoW — F1
superseded: rollback/replay is moot when commit is one atomic root flip)** →
K8b → K8c → F3 → F4 → **F5 (BeFS surpassed)**. The remaining F-arcs are
host-native — ideal parallel work needing no hardware session.
