...
| **FACET-1** ✅ landed 2026-07-12 (`us-facet1`; eye-witness attended-pending) | The `apps/facet` viewer vessel on the pulse/phonolite single-view idiom: `facet <image>` reads a file, decodes via `lux` (`decode` dispatch on magic bytes), packs the linear `RgbBuffer` through the sRGB OETF to 8-bit RGBA, and shows it aspect-fit/centered in a quartzite window via a new additive `platforms::macos::image_view` module (CPU blit through `NSBitmapImageRep`, tagged sRGB for color-managed display; rescales with the window). Euclase textured-quad (GPU) path, pan/zoom, and pixel readout are the later arcs | direct read |
