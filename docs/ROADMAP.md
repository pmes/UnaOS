...
| **LOGIN** | A human user on UnaOS (RULINGS R51): `fs/users.rs` (`USERS.DAT` record store — name, salted SHA-256 credential, home path), the `user:<name>` SESSION PRINCIPAL stamped into the loader's own per-slot principal on both arches and enforced by the existing `SYS_OPEN` owner/grants check, `login`/`logout`, the M1 fixture. **M1 + M2 fixed-unflown on `exec-orin-login`** (2026-09-12: `/home/<user>` at first login on the EL0 FAT volume, the knob-on aarch64 path-taking `sys_open`, the home ACL proof on both arches; fixtures behind `UNAOS_LOGINST=1`); M3 the login screen (`UNAOS_LOGIN=1`), M4 Log Out in the crystal follow. Blocked-on-record: one `HOST_VERBS` row (`libs/sys/midden_core`), one builder map line (`builder/src/main.rs`); LEDGER SO32 | jetson (shared code, both arches) |
