...
pub fn run(console: &mut Console, pal: &mut TargetPal) {
    ...
    report(&mut pager, console, pal, &mut tally, "video.geometry", test_video_geometry()); #[cfg(all(feature = "sntp6", feature = "net6", target_arch = "aarch64"))] crate::net_sntp_client::service_tick(); #[cfg(all(feature = "sntp6", feature = "net6", target_arch = "aarch64"))] report(&mut pager, console, pal, &mut tally, "net.sntp", match crate::net_sntp_client::fixture() { Ok(()) => Outcome::Pass, Err(w) => Outcome::Fail(String::from(w)) }); // FONTAA (SO48) — SAME-LINE append, deliberately: a new statement on its own line here shifts every `panic::Location` below it in this file and moves the knob-off image (LAWS §5, "cfg does not protect byte identity"). The fixture body is at this file's TAIL, where an append shifts nothing. The call goes BEFORE the `//`, or it compiles nothing and this gate stays green while measuring an absent fixture (LEDGER P7).  ORINTIME/SNTP-NET6 appended after FONTAA on this SAME line for the identical reason: SNTP-NET6, folded LINE-NEUTRAL (code before the comment, LEDGER P7 — a new line here would shift every `panic::Location` below it in a file every image compiles). TWO statements, in this order and for two different reasons. FIRST `service_tick()`: the client's NIC-AGNOSTIC drive seam, latched and guarded, so an operator at the Orin console can reach the live sync with a word they already type — it declines silently when the bring-up has not settled, costs one relaxed atomic once it has run, and emits its own bounded `[sntp6]` witness. It is NOT a test and takes no row. SECOND the deterministic `fixture()`, which snapshots the civil clock (including anything the live tick just anchored), drives canned datagrams through the SHARED parser and the SHARED anchor path, asserts the bar's honesty rule in both directions, and restores the clock exactly as it found it (`b3408a24` is the commit that had to teach the x86 fixture that last part). The pump inside `service_tick` is bounded by iteration count, not by a sync primitive, so the coordinator-never-blocks rule in this module's header still holds.  ⚠ FONTAA MOVED OFF THIS SUITE 2026-09-22 (TOOLRESCUE): `video.font.aa` was a leg of the INTERACTIVE `tste` suite, which ships in every image and is executed by NEITHER `./arroyo test` NOR `test-arm` — GATEHYG's fixture-reachability gate named it on first contact. The leg now rides the boot wire from `shell.rs::font_aa_witness`, per this file's own prescription below; `test_font_aa` is `pub(crate)` for that caller. The REMOVAL is same-line too, for the same panic::Location reason the append was.
...
pub(crate) fn test_font_aa() -> Outcome {
