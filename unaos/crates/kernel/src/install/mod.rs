    ...
    fn read_sectors(&self, lba: u64, buf: &mut [u8]) -> Result<(), InstallError> {
        ...
        for (i, chunk) in buf.chunks_mut(SECTOR).enumerate() {
            let n = match self.handle {
                ...
                block::BlockHandle::SdMmc => Err(block::BlockError::NotReady), #[cfg(all(target_arch = "x86_64", feature = "ahci", not(feature = "ahci-write")))] block::BlockHandle::Ahci { .. } => Err(block::BlockError::NotReady), #[cfg(all(target_arch = "x86_64", feature = "ahci-write"))] block::BlockHandle::Ahci { port } => block::read_block_ahci_port(port, lba + i as u64, chunk), // AHCIBOOT + AHCIWRITE, and the two polarities differ HERE and only here. Unarmed (AHCIBOOT): the installer can never so much as look at a SATA disk. Armed (`ahci-write`): the READ arm opens, because the pre-flight census IS the guard — `partition::census` must read the whole GPT and probe every partition's first 8 KiB before anything decides a target, and a guard that cannot read the stranger's disk cannot refuse it either (rmbp-ledger B91's asymmetry, pointed the right way). Reading mutates nothing; the WRITE arm below stays a refusal in BOTH polarities, so opening this one grants no write anywhere.
            }
            .map_err(map_blk)?;
            if n < chunk.len() {
                return Err(InstallError::Io);
    ...
    fn write_sectors(&mut self, lba: u64, buf: &[u8]) -> Result<(), InstallError> {
        ...
        for (i, chunk) in buf.chunks(SECTOR).enumerate() {
            match self.handle {
                ...
                #[cfg(all(target_arch = "aarch64", feature = "tegra", feature = "sdmmc"))]
                block::BlockHandle::SdMmc => Err(block::BlockError::NotReady), #[cfg(all(target_arch = "x86_64", feature = "ahci"))] block::BlockHandle::Ahci { .. } => Err(block::BlockError::NotReady), // AHCIBOOT + AHCIWRITE: THIS ARM STAYS A REFUSAL IN EVERY CFG, `ahci-write` included, and that is the point of the arc's shape. A whole-DISK target on SATA can address LBA 0 and both GPTs, which on the bench rMBP is Catalina's partition table; so the disk-wide `InstallTarget` never writes a SATA sector. The one SATA write path is `partition::PartitionTarget`'s, which carries a `block::WriteGrant` bound to one partition's LBA range and calls `block::write_sectors_granted` instead of this match. The block layer refuses one layer down too (`write_block_ahci` returns `NotReady` in every cfg), so this is still the second of two refusals.
            }
            .map_err(map_blk)?;
        }
        Ok(())
