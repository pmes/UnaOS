...
impl XhciController {
    ...
    /// string + tier depth), read the device descriptor, and dispatch: a downstream device that is
    /// itself a hub (class 0x09) is queued into `hubs_pending` so the next `service_hubs` pass
    /// brings it up and descends another tier (this is what makes the walk recurse past tier 1);
    /// an HID keyboard/mouse hands off to the existing endpoint-configuration path. Storage /
    /// other non-HID leaf devices behind a hub are still not handled.
    ///
