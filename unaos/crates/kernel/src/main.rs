...
fn tegra_early_stop(boot_info: &'static mut BootInfo) -> ! {
    ...
    #[cfg(all(target_arch = "aarch64", feature = "sdmmc"))] unaos_kernel::drivers::block::set_boot_volume_serial(boot_info.boot_volume_serial); unaos_kernel::arch::memory::init(boot_info); // ORIN-BOOTID — THE TEGRA PUBLISH, and without it the whole comparison is inert on this board. The FRGUARD publish at main.rs:263 is UNREACHABLE here: `kernel_main` calls `tegra_early_stop(boot_info)` at :190 and that fn is `-> !`, so every line of `kernel_main` below :190 is dead on a tegra image (its own `allow(unreachable_code)`, and the comment at :184 states it). MEASURED, not argued: `~/unaos-bench/capture/line-acm0/orin.log` carries the loader's `boot volume FAT serial 0xde001a13` and `[sdmmc] root mount … vol_id=0xabfbdefa` on the same flight — the kernel held both identities and could compare neither, because the cell they would meet in was never written on this board. Placed on THIS statement because `memory::init` consumes the `&'static mut BootInfo`; the read must precede it and this is the last line where it can. Statement BEFORE the first `//` (SO8: a statement appended after a comment marker is comment text, and every gate still passes). ⚠ LINE-NEUTRAL fold — no `panic::Location` in this file moves and the knob-off image is byte-identical.
