...
fn kernel_main(boot_info: &'static mut BootInfo) -> ! {
    ...
    if unaos_kernel::arch::gic::is_v3() {
        ...
        unaos_kernel::arch::exceptions::install(); virt_el0_start_maybe(); #[cfg(feature = "virt_tick")] unaos_kernel::arch::timer::virt_tick_start(); // VIRTPREEMPT (orin 17) — the EL1 PERIODIC TICK, folded onto this line for the same `panic::Location` reason as its neighbours, and LAST on it on purpose: `virt_tick_start` ends by unmasking IRQ permanently, so every statement before it (the vector install, the user-window install, the EL0 spawns) is staged with IRQs still masked and the first tick cannot land inside `mmu_tegra_el0::install()`. It must be AFTER `exceptions::install()` (VBAR_EL1 has to point at the real table before an IRQ is taken at EL1) and AFTER `boot_virt::drop_to_el1`, whose `msr cntp_ctl_el0, xzr` this deliberately does NOT remove — the drop still disarms and this re-arms from EL1, exactly the JM6/ORIN-BSPTICK pairing, which the same drop's CNTHCTL_EL2.EL1PCTEN|EL1PCEN makes legal. `boot_virt.rs` is unchanged by this arc. See timer.rs tail §VIRTPREEMPT. JV1 — EL0 ON VIRT, staged HERE and nowhere else. It must be AFTER `exceptions::install()`, which is what puts the real `VBAR_EL1` in place for the `SVC` the EL0 task takes, and AFTER `boot_virt::drop_to_el1`, because `mmu_tegra_el0::install()` hangs its user window off the LIVE `TTBR0_EL1` and that root is not the EL1 one until the drop. It must be BEFORE `run_capstone_boot_core`, which never returns. Pre-staging onto the queue is the SUPPORTED shape, not a workaround: `capstone_queue_pre_staged` exists precisely so the two priority witnesses (which DRAIN the queue) stand down when tasks are already staged, so the EL0 task cannot be eaten before CAPSTONE runs. FOLDED onto this line, and code-before-comment, for the `panic::Location` reason this file's other tail-defined knobs cite — a source line added here shifts every later Location in `main.rs` and moves the knob-off `kernel8.img` hash.
...
fn tegra_early_stop(boot_info: &'static mut BootInfo) -> ! {
    ...
    match unaos_kernel::arch::fdt_tegra::bpmp_geometry(
        ...
        Some(geom) => {
            ...
            if let Some(chan) = unaos_kernel::arch::bpmp_tegra::jb1b_ping(&geom) {
                ...
                } #[cfg(feature = "jd1dc")] unaos_kernel::arch::display_tegra::jd1_dc_probe(&chan, boot_info.dtb_addr, boot_info.dtb_size, mmu.ram_gib_mask); #[cfg(feature = "ga10bprobe2")] unaos_kernel::arch::ga10b_probe::ga10bprobe2_run(&chan, boot_info.dtb_addr, boot_info.dtb_size, mmu.ram_gib_mask); #[cfg(feature = "ga10bprobe3")] unaos_kernel::arch::ga10b_probe::ga10bprobe3_run(&chan, boot_info.dtb_addr, boot_info.dtb_size, mmu.ram_gib_mask); #[cfg(feature = "ga10bprobe1")] unaos_kernel::arch::ga10b_probe::ga10bprobe1_run(&chan, boot_info.dtb_addr, boot_info.dtb_size, mmu.ram_gib_mask); // JD1-DC (UNAOS_JD1DC=1, default OFF) — the BPMP-guarded, READ-ONLY nvdisplay register probe; see the JD1-DC tail block in display_tegra.rs. THIS instruction is the only point in the boot where both of its ordering constraints hold: BPMP-FIRST (the `chan` this borrows was established by `jb1b_ping` ~60 lines up, and without it the MRQ_PG GET_STATE guard that earns the first read cannot be asked — which is why the probe could NOT stay at its old site inside `jd1_survey`), and JD1-FIRST (the scanout resolution, `map_fb_region`, `fbcon::init` and the `WRITER` seed are ~90 lines up, so panel + serial + shell are already live on a framebuffer resolved by a pure DTB RAM walk and a probe that goes wrong costs the experiment, not the boot). Last in the BPMP block so every other diagnostic has already reached the wire before the one read that could end it. Appended to this line, never a new one: knob-off it is cfg-erased and no panic `Location` below moves. GA10B-PROBE1 (UNAOS_GA10B_PROBE1=1, default OFF) — the first read-only GA10B iGPU probe rung: it borrows the SAME `chan` `jb1b_ping` established (BPMP-first: the rail gate that earns the first BAR0 read cannot be asked without it), reads the risk-ordered register list announce-first, and ENDS THE BOOT in PSCI SYSTEM_OFF (`power::shutdown`) per the cold-boot bench law — so it never returns and no later boot phase runs on a probe flight. Its own media by design (co-arming with other tegra knobs is not a supported flight). Appended to this line, never a new one: knob-off it is cfg-erased and no panic `Location` below moves. GA10B-PROBE2 (UNAOS_GA10B_PROBE2=1, default OFF; a SIBLING knob, not a dependent) — rung 2 of the GA10B ladder (docs/dev/evidence/orin14/GA10B-LADDER.md): BPMP power-on + clock-enable of the gpu@ node's domain and clocks, one PMC_BOOT_0 read behind an explicit pg=ON readback, symmetric restore, and it RETURNS — so it sits BEFORE rung 1's call (a co-armed image runs rung 2 then rung 1's SYSTEM_OFF; the rung-2 flight arms rung 2 alone and boots through to the desktop). Appended to this line, never a new one. GA10B-PROBE3 (UNAOS_GA10B_PROBE3=1 for rung 3 alone, =2 for rung 3 + rung 3b; default OFF; a THIRD sibling knob) — rungs 3 and 3b of the same ladder: the SAME BPMP power+clock bracket rung 2 proved, then the read-only pass over what the platform firmware left behind (fuses -> MC -> TOP -> GSP falcon v1 -> GSP priscv BCR -> PMU falcon2, the one new read aperture, LAST), and under `ga10bprobe3b` the ladder's FIRST GA10B MMIO WRITES (GSP engine reset assert/deassert, then one MAILBOX0 scratch write) — then symmetric restore and it RETURNS, like rung 2. It sits between rung 2's call and rung 1's for the same reason rung 2 does: rung 1's SYSTEM_OFF must never run in front of a rung that has to reach the desktop. GA10B-PROBE4 does NOT run here: its DMA window is seated by heap-guard (`memory::init`, ~60 lines below), which this board runs AFTER the BPMP block — the 4a boot 1 of 2026-09-11 read REFUSED reason=no-dma-window with the [ga10b4nc] census printing after the rung. Its call is appended to the post-heap-init line below, on a channel reopened from the same geometry without a second IVC sync.
            }
        }
    ...
    //    the `tegrasmp` `start_secondaries` runs above (`UNAOS_NOTEGRASMP=1` is the opt-out), CPU_ON succeeds on
...
fn jd2_console_pump(_arg: usize) {
    ...
    loop {
        ...
        if pending_rel.is_some() || pending_abs.is_some() {
            ...
            #[cfg(feature = "orinclick")] unaos_kernel::arch::display_tegra::orin_drag_steer(pal.width() as i32, pal.height() as i32); cursor::draw_over(&mut pal); // DRAGDEAD (A27) — STEER a live title-bar grab from the ORIN's ONLY pointer drain. `wm` mints the grab (`drag_begin`, via `wc_click_route`'s chrome arm) and ends it (`drag_end`, on the release edge) but nothing on this board ever fed it MOTION, so `[wm-act] drag-end … -> no-move` four times in render6 boot 2 — `DRAG_MOVES` never left zero because `drag_motion` was never called. The x86 drains reach it through `wc_route_tail` (main.rs:1991/6725) and the Pi's through `wm::drag_route_tail` (main.rs:3879/5442/5452, `route_input_to_active_el0` and `render_service`); the Orin runs NEITHER — this pump is its pointer path and it only moved the arrow. PLACED AHEAD OF `draw_over` DELIBERATELY, and both halves matter: after `move_rel`/`set_abs` so `pal::cursor::pos` already carries this frame's travel (the window must not trail the arrow by a frame), and before the arrow goes back on the glass so a compositor pass cannot land under it and make the next `cursor::restore` stamp a stale cursor-sized patch over the moved window. Once per FRAME, not per report — the pump has already coalesced every report of the frame into `pending_rel`/`pending_abs`, and `wm`'s own pacer (`drag_motion_paced`) is the second gate. Idle cost: one relaxed load of `DRAG_WIN`. Geometry comes from the `Screen` this task is already holding, so no `WRITER` lock is taken at this seam (LOCKFIX `7847ceea`) — which is why `wm::drag_route_tail` itself is NOT the call: it reads the panel through a blocking `*WRITER.lock()` one line above the pacer. ⚠ SAME-LINE fold, line-NEUTRAL — knob-off this statement is `#[cfg]`-erased and the line is `cursor::draw_over(&mut pal);` verbatim.
            needs_render = true;
...
fn pump_usb_into_gui() {
        ...
        while n < buf.len() {
            match unaos_kernel::pal::peek_event_uncounted() {
                Some(ev) => {
                    ...
                    #[cfg(feature = "desktop_firmware")] if unaos_kernel::video::strip::key_escape(ev) { unaos_kernel::pal::note_uncounted_discard(1); continue; } let before = unaos_kernel::arch::aarch64::syscall::user_input_active(); // APPESC (KEYDOORS F3) — THIS BRANCH HAD <TAB> BUT NOT <Esc>. A10 folded `strip::key_escape` onto the bare-shell drain below (:4578) and stopped there, so on the Pi with a full-screen KERNEL app on the panel and focus 0, <TAB> still cycled but <Esc> could not dismiss an open bar or SHARD menu: it was re-buffered and requeued to the app, exactly as it reached `gui_send` on the other branch before A10. One key, one branch, one board — the narrowest of the KEYDOORS findings and the last of A10's three doors. Placed AHEAD of the `let before`/`wc_shell_focus_key` pair on the modal-first rule both arch routers follow (a modal surface must get the key before the focus ring can TAB the desktop out from under an open menu). ⚠ THE `note_uncounted_discard(1)` IS NOT DECORATION AND IS WHY THIS FOLD IS NOT A COPY OF :4578's. This loop peeks off the UNCOUNTED seam (`peek_event_uncounted`), so an event that leaves the pipeline and is never requeued must be accounted, or `[uvug10] evq`'s `push - drop - pop` occupancy reads permanently high by one per swallowed <Esc>. The `wc_shell_focus_key` swallow arm three lines below already pays exactly this, for exactly this reason; the COUNTED-`next_event` twin at :4578 owes nothing and correctly says nothing. ⚠ FOLDED onto the `let`, code before comment (F0's rule).
    ...
    while let Some(ev) = unaos_kernel::pal::next_event() {
        ...
        #[cfg(feature = "desktop_firmware")] if unaos_kernel::video::strip::key_escape(ev) { continue; } #[cfg(feature = "desktop_firmware")] if unaos_kernel::video::quarry::key_route(ev) { continue; } let before = unaos_kernel::arch::aarch64::syscall::user_input_active(); // QUARRYDOOR (KEYDOORS F1) — the PI's half of Quarry's missing shell door. `quarry::key_route` (Esc never closes — R24; `0x1C`..`0x1F` navigate, Enter opens, Backspace ascends, `r` re-reads, Wheel scrolls) had exactly ONE caller in the tree — `arch/aarch64/syscall.rs:13211`, the EL0 RING door — so on the path a Pi desktop boot with focus 0 actually takes, every one of those keys fell through to `gui_send` and the file manager could not even be closed from the keyboard. Same statement, same shape, same reason as TABKEY and A10 before it. Ordered AFTER `strip::key_escape` (a bar or SHARD menu composites above Quarry's window, so the modal surface wins) and BEFORE `wc_shell_focus_key` (an open file manager eats its own arrows before the focus ring sees them) — the order both arch routers already follow. No focus test here on purpose: `key_route` gates on `focus_asid() == OWNER && on_glass()` since SO9FIX 63b109f6 (was `on_glass()` alone — SO9) (`video/quarry/live.rs:1651`), returning false unless Quarry's row is live AND above `wm::shell_z()`, so a closed or buried Quarry cannot steal an arrow from the shell. `continue`, not `break`: a consumed key moved no focus, so this loop's `gui_send` destination is still valid — the `break` below is for a real focus MOVE only. Events here come off the COUNTED `next_event`, so no discard accounting is owed (unlike the uncounted peek above). Gated `desktop_firmware` (`video/mod.rs:685`); `UNAOS_QUARRY` is handled inside the seam so this fold needs no second `cfg`. ⚠ FOLDED onto the `let`, code before comment. // A10 (MENUBAR2) — the PI's half of the same missing shell door. `strip::key_escape` was asked only at the EL0 RING door (`user_input_enqueue`) and in x86's `wc_route_event`; this bare-shell drain — the path a Pi desktop boot with focus 0 actually takes — asked nobody, so <Esc> reached `gui_send` and the open menu stayed down. Measured on the Orin's twin of this loop (render7 :2414 open, :2437 KEY 0x1b, :2487/:2526 still open, :2569 closed reason=outside); the Pi is the same code with the same gap, and the two are fixed together because R21's menus are ONE OS. Placed ahead of `wc_shell_focus_key` on the modal-first rule both arch routers already follow. Consumes only a bare <Esc> with a menu down, so every other boot is behaviour-alike. ⚠ FOLDED onto the `let` below, never given a line of its own: `kernel8.img`'s panic-`Location` byte-identity proof names this file.
        if unaos_kernel::arch::aarch64::syscall::wc_shell_focus_key(ev) {
            if unaos_kernel::arch::aarch64::syscall::user_input_active() != before {
                break;
            }
...
fn render_pass<W: RenderWait>(w: &mut W) {
    ...
    loop {
        ...
        console_launch_drain("render_service"); if strip_dirty {
            unaos_kernel::ui_status::draw(&mut pal);
            dirty = true;
        } else if strip_tick {
        ...
        #[cfg(feature = "desktop_firmware")]
        if desktop
            && { shellwin_service_rearm(&mut shell_id, &mut shell_declined); shell_id == unaos_kernel::video::wm::WIN_NONE } // APPPIN (R49) — ⚠ SAME-LINE fold, line-NEUTRAL. The shell app's QUIT and LAUNCH on this body, inside the mint arm's own guard: a quit (the row gone under `shell_id`) clears the stale id and parks the arm SHUT (`shell_declined = true`); a launch (the dock's post) opens it (`shell_declined = false`) and the arm below mints on this pass through `open_shell_window`, rebinding the whole tuple. Sited on the `shell_id` term rather than above the `if` so no line is added; `desktop` is evaluated first and short-circuits. See the APPPIN block at this file's tail; the retired `pi_shell_reopen_drain` is deleted.
            && !shell_declined
            && unaos_kernel::video::desktop_firmware::armed()
        {
            ...
            match open_shell_window(info.width, info.height) {
                Some((store, fb, id)) => {
                    ...
                    shell_id = id; #[cfg(feature = "witness")] SHELLUP_SHELL.store(id as u64, core::sync::atomic::Ordering::Relaxed); unaos_kernel::video::dock::app_launched(unaos_kernel::video::dock::PinnedApp::Shell, id, "render_service", 1); // APPPIN — the mint arm IS this body's launch (boot and every tile relaunch), reported through the dock's one grammar. ⚠ FOLDED, CODE FIRST. // SHELLUP — publish the shell window's launch outcome for the `[shellup]` census. `shell_id` is a task LOCAL and the census runs on the input core, so without this store the one tenant the arc is named for is the one tenant the census could not name. ⚠ FOLDED, `#[cfg]` on the statement — PARITY §5.3.
...
fn x86_render_service(cpu: usize) {
    ...
    let mut shell_pal = unaos_kernel::pal::TargetPal::new(&mut shell_screen);
    #[cfg(feature = "wc")]
    let mut shell_dirty = false; #[cfg(feature = "wc")] let mut shell_gen = unaos_kernel::video::wm::winid_gen(shell_id); // APPPIN — the generation the live tuple was minted on: the quit arm's recycled-slot fence (an id is a slot alias; the (id, gen) pair is the identity). ⚠ FOLDED, `#[cfg]` on the statement — line-neutral.
    ...
    loop {
        ...
        loop {
            ...
            match ev {
                ...
                unaos_kernel::pal::Event::Button(_mask) => {} unaos_kernel::pal::Event::Action(a) => { #[cfg(feature = "wc")] { if desktop && shell_id != unaos_kernel::video::wm::WIN_NONE { if unaos_kernel::video::clipboard::terminal_action(a, &mut shell_console.current_input, &mut shell_console.sel).1 { shell_console.draw_input_line(&mut shell_pal); shell_dirty = true; } } else if unaos_kernel::video::clipboard::terminal_action(a, &mut console.current_input, &mut console.sel).1 { console.draw_input_line(&mut pal); } } #[cfg(not(feature = "wc"))] { if unaos_kernel::video::clipboard::terminal_action(a, &mut console.current_input, &mut console.sel).1 { console.draw_input_line(&mut pal); } } } // TERMSEL — the consumer now takes the console's line and SELECTION by `&mut` (disjoint fields) instead of a clone of the line: `Cut` removes cells from the line and the selection actions move `Console::sel`, and the returned flag says the input line changed, so this arm repaints it with the same `draw_input_line` a keystroke uses (and marks the shell window dirty on the crispy desktop, as the `Event::Key` arm does). A paste still re-enters through the ring AFTER this borrow ends, so the clone APPCLIP took for that reason is not needed. Same console predicate as before. // APPCLIP (R61) — **THE FIRST CONSUMER OF A DESKTOP ACTION, AND IT IS THE TERMINAL.** Until this arc `⌘C`/`⌘V`/`⌘X`/`⌘A` were resolved by the theme’s table at the HID decoder and then dropped (keymap.md §6). This is where they land on the x86 `wc` lane: the same drain, the same `wc_route_event`, the same match as every key and click — an action reaches this arm only when no focused ring-3 window took it, which is exactly the condition under which the keyboard belongs to the SHELL, so no focus test is added here and none may be. `clipboard::terminal_action` copies the SELECTION, or the current input LINE when none is live (TERMSEL, `clipboard.md` §7), and pastes by pushing the clipboard back onto the ring as `Event::Key`, which the `Event::Key` arm at the head of this same match then types and echoes — so a paste and a typed burst are the same code from here down, and `dirty`/`shell_dirty` are set by that arm exactly as they are for hands on the keyboard. `Cut` removes the selection (declined on the wire with none), `SelectAll` selects the line (TERMSEL). It picks its console with the same predicate the `Event::Key` arm above uses (shell WINDOW on the crispy desktop, backdrop console otherwise); duplicating the predicate rather than hoisting it keeps this fold to one line. No `Ctrl-C` case exists here or anywhere on the path — the table never claims it, which is R61’s whole point. ⚠ FOLDED onto the `Button` arm and given no line of its own — `main.rs` is compiled into the knob-off `kernel8.img` whose byte-identity is this track’s standing proof and `panic::Location` records embed line numbers (PARITY §5.3); CODE FIRST, then the prose (A10FIX).
        ...
        // `wc_click_route_at` -> `dock::press_at`, which POSTS `PinnedApp::Shell`). No reopen route,
        // no latch that remembers an id: the tuple below IS the instance and its life is the row's.
        // Replaces SHELLWIN-REOPEN (the SO1/SO10 mechanisms, retired by design).
        //  * QUIT — the row this tuple was minted for is gone (disc, app-menu Quit or
        //    `wc_close_furniture`, all through `wm::close`; the generation term also catches a slot
        //    re-issued to a stranger): free the store, drop the `Console` and everything typed into
        //    it, rebind an empty `Screen`/`TargetPal`, publish the slot-0 sink UNBOUND. The pin tile
        //    is already on the strip (`dock::pin_shell` sees no `KERNEL_OWNER_DESKTOP` row).
        //  * LAUNCH — a posted press. A live instance (scan-to-press race) is RAISED, never doubled;
        //    otherwise a FRESH one is minted through `open_shell_window` + `Screen::direct`, the exact
        //    path bring-up used (never `Screen::new` — GR26's OOM): new store, new `Console`, new row
        //    (new id and generation), first prompt presented owner-fenced, keyboard handed back.
        //    NOT gated on `desktop`: the tile exists wherever a dock composites, and the QEMU witness
        //    gate drives this arm with no takeover (`dock::apppin_selftest`). A DECLINE mints nothing; `open_shell_window` names it, the tile stays.
        #[cfg(feature = "wc")]
        {
            use unaos_kernel::video::dock::{self, PinnedApp};
            use unaos_kernel::video::wm;
            let row_gone = shell_id != wm::WIN_NONE
                && (wm::owner_of(shell_id) != Some(wm::KERNEL_OWNER_DESKTOP)
                    || wm::winid_gen(shell_id) != shell_gen);
            if row_gone {
                dock::app_quit(PinnedApp::Shell, shell_id, shell_gen, "x86_render_service");
                _shell_store = alloc::vec::Vec::new();
                shell_console = unaos_kernel::console::Console::new();
                shell_screen =
                    unaos_kernel::video::Screen::direct(unaos_kernel::video::FrameBuffer::new());
                shell_pal = unaos_kernel::pal::TargetPal::new(&mut shell_screen);
                ...
                shell_gen = 0;
                shell_dirty = false;
                unaos_kernel::arch::x86_64::syscall::shell_key_sink_note(desktop, false);
            }
            if dock::take_launch(PinnedApp::Shell) {
                if shell_id != wm::WIN_NONE {
                    unaos_kernel::arch::x86_64::syscall::user_input_set_active(0);
                    wm::focus_changed(wm::KERNEL_OWNER_DESKTOP);
                    dock::app_raised(PinnedApp::Shell, shell_id, "x86_render_service");
                } else {
                    let info = front_fb.info();
                    if let Some((store, fb, id)) = open_shell_window(info.width, info.height) {
                        _shell_store = store;
                        shell_console = unaos_kernel::console::Console::new();
                        shell_console.mark_in_window();
                        shell_screen = unaos_kernel::video::Screen::direct(fb);
                        shell_pal = unaos_kernel::pal::TargetPal::new(&mut shell_screen);
                        shell_id = id;
                        shell_gen = wm::winid_gen(id);
                        shell_console.draw(&mut shell_pal);
                        shell_pal.render();
                        let _ = wm::present_outcome_owned(id, wm::KERNEL_OWNER_DESKTOP);
                        unaos_kernel::arch::x86_64::syscall::user_input_set_active(0);
                        wm::focus_changed(wm::KERNEL_OWNER_DESKTOP);
                        unaos_kernel::arch::x86_64::syscall::shell_key_sink_note(desktop, true);
                        shell_dirty = false;
...
fn shellup_census_maybe() {
    ...
    #[cfg(feature = "desktop_firmware")]
    let armed = unaos_kernel::video::desktop_firmware::armed();
    #[cfg(not(feature = "desktop_firmware"))]
    let armed = false;
...
fn jd2_supstate_phase2(
    ...
    ); serial_println!("[supstate] sweep services=prtscr:{} tenant:{} ladder:{} rast:{} serialrx:{} click:{} poll=present,sweep -> MIRRORED", cfg!(feature = "holocron"), cfg!(feature = "orintenant"), cfg!(feature = "orinladder"), cfg!(feature = "rast"), cfg!(feature = "orinrx"), cfg!(feature = "orinclick")); // SUPPRTSCR (orin 17) — printed ONCE, beside the `[supstate] roles ... -> SPLIT` line, because the thing it witnesses is a STATIC PROPERTY of the build: which of `jd2_console_pump`'s sweep services this copy of the sweep actually compiles. `cfg!` is a runtime bool, not a `#[cfg]`, so every flight prints the WHOLE set with each term's true/false — an armed knob that reads `prtscr:false` convicts the mapping, and a mirror deleted by a later edit convicts itself without needing a Print Screen press to notice. The one line the next `supstate` flight needs to prove this fix flew; `poll=present,sweep` names the two re-arm folds. ⚠ LINE-NEUTRAL fold.
    ...
    loop {
        ...
        if cntpct().wrapping_sub(last_sweep) >= sweep_ticks {
            last_sweep = cntpct();
            unaos_kernel::vugras::idle_sweep(sweep_tick); unaos_kernel::arch::display_tegra::sup_present_census(sweep_tick); #[cfg(feature = "rast")] unaos_kernel::arch::display_tegra::orin_rast_census(sweep_tick); #[cfg(feature = "orinrx")] unaos_kernel::arch::serial::serialrx::census(sweep_tick); #[cfg(feature = "orintenant")] unaos_kernel::arch::display_tegra::orin_tenant_census(sweep_tick); #[cfg(feature = "orinladder")] unaos_kernel::arch::display_tegra::orin_ladder_census(sweep_tick); #[cfg(feature = "holocron")] unaos_kernel::video::prtscr::service(); // SERIALRX (ORINRX) — the `[serialrx] rx=` census on the `supstate` sweep. ⚠ LINE-NEUTRAL append. // ORIN-SUPSOUND — the ~10 s presenter census, authored by the INPUT SOURCE on purpose: this board's UART is shared with the SPE's TCU and drops bytes, so a dead presenter must produce a REPEATING LINE (`pass=+0 -> DEAD`), never a silence. Appended to an existing statement, and un-`#[cfg]`ed because this whole block is already `supstate`-gated. ORIN-RASTGLASS: the rast census is the phase-2 twin of the sweep-site call in `jd2_console_pump`'s phase-1 loop and it was MISSING from this copy, so on a `supstate` image the census stopped dead at the phase-1 boundary and the rung's verdict was whatever the last phase-1 sweep had said — with `RG_CONSOLE_OWNS` never latched (see the phase-2 boundary line above), permanently pre-takeover. The census self-terminates on `RG_DONE`, so restoring it here costs one relaxed load per sweep once the question is answered. ⚠ LINE-NEUTRAL append. // SUPPRTSCR (orin 17) — THE THREE SERVICE CALLS THIS COPY OF THE SWEEP NEVER CARRIED, and the first of them is a live operator defect, not a witness hole. `jd2_console_pump`'s sweep (main.rs:3014) runs `prtscr::service()`; this one did not, and `jd2_supstate_phase2` NEVER RETURNS, so on a `supstate` image the legacy phase 2 is dead code and there is no other service site on the board — the xHCI decoder arms `prtscr::request()` on the HID 0x46 press edge exactly as it does on every other image, and the flag then sat armed forever. That is the pre-PRTSCR-ORIN defect restored in the clone, and it is invisible on a `path=jd2-console-pump` boot (render8's), which is why four Print Screen captures came back green. `orin_tenant_census` and `orin_ladder_census` have EXACTLY ONE caller each in this tree (main.rs:3014), so `UNAOS_ORINTENANT=1` / `UNAOS_ORINLADDER=1` were INERT on a `supstate` image — the ORIN-RASTGLASS omission shape this function's own header (main.rs:7513) names as the drift it must detect. Gates unchanged: `prtscr` rides `holocron` for the same reason it does at :3014 (the repo's "this boot may WRITE its boot medium" arming knob, no code dependency), and all three are `#[cfg]`-gated so the knob-off kernel8.img is byte-identical. ⚠ LINE-NEUTRAL fold, and SO8: the three statements go BEFORE the first `//` on this line — appended after it they would have been comment text and every gate would still be green.
            #[cfg(feature = "orinclick")]
            unaos_kernel::arch::display_tegra::orin_click_census(sweep_tick);
            sweep_tick += 1; if let Ok(mut x) = unaos_kernel::drivers::xhci::claim() { x.poll_events(); } // SUPPRTSCR (orin 17) — the `supstate` mirror of PRTSCLOST's sweep-tail re-arm (jd2_console_pump, same statement). ONE Normal TRB is outstanding on the keyboard interrupt-IN and it is re-armed only from the completion dispatch, while SET_IDLE duration 0 means a report that is not collected is never resent — so every state change between the TD retiring and the re-arm is gone with no trace. The block above now runs `prtscr::service()`, whose slice budget is ~75 ms on this board and can carry past 110 ms on a 32 KiB `write_grow`; that hole is wider than a fast press-and-release. `claim()` is try-based, so a contended pass is a no-op and the pass behaves exactly as before. ⚠ LINE-NEUTRAL fold.
        }
        unaos_kernel::arch::sched::yield_now();
    }
}
...
fn orin_render_service(_: usize) {
    ...
    loop {
        ...
        dirty |= unaos_kernel::ui_status::tick(&mut pal); #[cfg(feature = "desktop_firmware")] unaos_kernel::video::pulsewin::service(); #[cfg(feature = "desktop_firmware")] unaos_kernel::video::quarry::service(); console_launch_drain("orin_render_service"); #[cfg(feature = "holocron")] unaos_kernel::video::prtscr::service(); // APPPIN (R49) — the DOCK'S CONSOLE launch drains here (`dock::console_launch_service`, arch-neutral: raise-if-live, else `fbcon::panel_console_window_open`, retrying a transient decline and naming the count when it gives up — render13's `route=declined` had no reason line, so a decline now costs 32 quiet passes before it is reported). The SHELL's launch is NOT drained here: the console pump owns the shell instance and drains it on its own pass (the APPPIN block at this file's tail). The retired `orin_shell_reopen_drain` (SO10's re-mint, and one of SR6's two board-named drains) is deleted, not cfg'd off. ⚠ FOLDED onto this line, never added below it — panic `Location`s. // PULSEWIN — the Pi's fold, restored (DESKSCENE): same cadence, same envelope, never a second sample. QUARRY-ORIN (A8) — and the file manager's latch drains beside it. `quarry::service()` is `reap_jobs()` + one `AcqRel` swap on a quiet pass, so this costs a relaxed load per pass and nothing else; when the latch IS set it calls `open()`, which reads directories — legal HERE and not in the click router (the reason `dock::press_at` latches instead of opening). The Pi drains it from ONE place, `arch/aarch64/syscall.rs`'s strip-press arm, which this board also compiles (`desktop_firmware`, and render6-boot2's `[dock] press … raised=true` proves that arm runs here) — but that arm fires only on a dock press, so `open()`'s own `reason=panel-busy` re-latch and `tegra_quarry_seat`'s would both sit unread until an operator happened to click the strip. This is the pass-driven drain that closes them. ⚠ FOLDED onto this line rather than added below it, for the reason PULSEWIN names.
...
fn tegra_desk_cascade() -> bool {
    ...
    let activated = desktop_firmware::activate(); #[cfg(feature = "login")] unaos_kernel::fs::users::screen_open_at_ignition(menubar::enabled(), activated); // SO43 — THE LOGIN SCREEN NEVER OPENED AT BOOT, AND THIS IS THE ONE SEAM ON THIS BOARD THAT COULD HAVE OPENED IT. LOGIN M3 wrote the gate `if activated`, and `activated` is `desktop_firmware::activate()`'s return, which is `fbcon::console_is_routed()` — a fact about where the CONSOLE's glyphs land, not about whether a desktop came up. Five render14 flights printed `[deskcascade] -> CASCADED windows=2 bar=1 owns_pixels=1 route=ROUTED activate=false`: the desktop up, the bar on, and the gate saying no. THE PREDICATE IS NOW THE DESKTOP EXISTING — `menubar::enabled()`, READ BACK off the bar `activate` has just claimed at its step 4 (the same discipline `activate` applies to `console_is_routed`, and the same value this function reads three lines below into `bar`, whose `!bar` arm REFUSES the cascade as `activate-declined`). `activated` is still handed to the seam and still printed there, so a capture says which of the two facts decided; see `fs::users::screen_open_at_ignition` for why both are arguments. AND THIS IS THE ONLY LIVE SITE ON A JETSON IMAGE: main.rs:6380 is inside `x86_render_service` (`#[cfg(target_arch = "x86_64")]`, main.rs:6362) and its aarch64 twin main.rs:5342 is inside `render_service` (`#[cfg(all(target_arch = "aarch64", feature = "baremetal"))]`, main.rs:5312) whose gate is UNSATISFIABLE here — `baremetal = ["pi", "aarch64_el0"]` (Cargo.toml:236) and `pi` + `tegra` is a hard `compile_error!` (`arch/aarch64/serial.rs:23`) — so neither is compiled, let alone run; the wire proves it too, the Shell row on this board is minted `by=console-pump` and no `[shellwin-pi]` line exists in either render14 capture. main.rs:7944 is the same defect in `tegra_desk_arm` (`tegradesk`) and is left untouched by this brief's file list — its stop-lines refuse before the line, and the exact one-word change is in the report. ⚠ LINE-NEUTRAL fold, CODE BEFORE COMMENT (LEDGER P7).
    #[cfg(feature = "witness")]
...
fn shellwin_window_open(pw: usize, ph: usize) -> Option<ShellWin> {
    ...
    // APPPIN — the band clear that stood here now belongs to the BOOT call site (`jd2` phase 2): it is the scene's first paint, and a tile RELAUNCH of the shell app must not repaint the panel.
    let wtop = unaos_kernel::ui_status::top_chrome_h(pw, ph);
    let workh = ph.saturating_sub(wtop);
    let cw = (pw / 2).clamp(320, pw.max(320));
...
// =================================================================================================
// APPPIN (R49 / R50, orin 27, 2026-09-12) — **the shell is a pinned APP on every render body.**
// =================================================================================================
//
// Peter, on the render13 glass (RULINGS.md R49): *"console should be an app that is pinned to the
// taskbar not this mystery thing that appears"* · *"same with shell"*. R50: *"quarry is our finder."*
//
// ## The model (one model, three bodies)
//
// A pinned app's tile is ALWAYS on the dock (`video/dock.rs`, APPPIN). While the app has a live
// window the tile is that window's row; while it has none the tile is a pin, and pressing it POSTS
// a launch (`dock::take_launch(PinnedApp::Shell)`). Closing the window — the red disc, the app
// menu's Quit, `wc_close_furniture` — is a QUIT: `wm::close` frees the row, and the body that owns
// the instance's state tears that state down on its next pass when it sees the row gone. A relaunch
// mints a NEW window (new id, new generation) through the SAME function the boot used for the first
// one. Nothing remembers a dead id; nothing re-mints over old pixels; there is no reopen route.
//
// The three render bodies differ in exactly ONE axis — WHO OWNS THE INSTANCE — and that is the whole
// of what differs below (SR6 named this axis when it counted the three reopen drains):
//
//  * `x86_render_service` — the tuple `(_shell_store, shell_screen, shell_console, shell_id)` is a
//    set of task locals in the drain loop, so its quit and launch arm is INLINE there (the APPPIN
//    block at the tail of the event drain; `open_shell_window` is the mint seam).
//  * the Pi `render_service` — the same tuple, with a MINT ARM that already knows how to rebind it
...
//
// ## What "quit" tears down, and where
//
// x86 and the pump free the surface store and drop the `Console` the pass they notice the row gone.
// The Pi's tuple is rebound by its mint arm, so its old store is freed at the NEXT launch rather than
// at the quit — the same instant the previous arc freed it, and the operator cannot tell the two
// apart (the window is gone either way); a teardown at the quit itself needs the mint arm's locals,
// which is a fold on the Pi's own line and pi's to take. The console's document (its glyph cells,
// CONSOLETEXT) survives a quit by design: the app's DATA outlives its window, its window does not.
//
// ## Keys with no shell instance on the cascaded scene
//
// [`shellwin_absent`] drops a keystroke when the scene is up and no shell window is live. Before
// this arc a key in that state painted the pump's `Console` onto the PANEL back buffer — the
// render4 SCREEN0.PNG defect (banner + prompt splashed over the desktop) that A19 fixed only while
// the window existed. With the shell an app, "no window" means "nothing to type into"; the tile
// launches one. A scene that is NOT up (no cascade) keeps the pre-A19 panel shell exactly as before.
//
// ## The wire (one grammar, every body, both apps — `video/dock.rs` prints it)
//
// ```text
...
// ```
//
// and for the console, from any body, `by=x86_render_service` / `render_service` /
// `orin_render_service` (the pass that drains it, unchanged names):
//
// ```text
...
// ```
//
// ## HISTORY — what stood in these lines, and why every piece of it is gone (not `cfg`'d off)
//
// SHELLREOPEN-ORIN (S4/A7, orin 16): `orin_shell_reopen_drain` — drained `dock::take_shell_reopen`
// from `orin_render_service` and reinstalled the CONSOLE route, because *"on the Orin the shell IS
// the routed console"*. render8 falsified the premise (SO10): the cascaded scene gives the shell its
// own row. SO10's fix kept the drain and re-pointed it at a RE-MINT: `tegra_shell_note` recorded the
// surface's base/len/extent/stride/origin at the mint, and `tegra_shell_remint` re-created a `wm`
// row over the SAME pixels the pump still owned (`[realdesk] shell-remint … -> MINTED`). render13
// boot 1 (evidence `docs/dev/evidence/orin27/render13-boot1-comp.log:604`) is what that looked like
// on the glass: the re-minted row composited, and every `[comp2] rollup` after it read `sprite_us=1`
// with `[cursor12] offer … -> nohit` — the pointer was lost over the re-minted window. Peter's
// ruling on that boot is R49. SHELLREOPEN-PI (orin 16 for pi 7): `pi_shell_reopen_drain` re-armed
// the Pi's mint arm off the same latch. CONSOLEPIN (orin 17): `console_reopen_drain` wrapped
// `dock::console_reopen_service`, which minted through `panel_console_window_open` and, on this
// boot, answered `route=declined -> DECLINE` twice with no reason in the capture — the "mystery
// thing" of R49's first sentence. The mechanisms are deleted here: the latches (`SHELL_REOPEN`,
// `CONSOLE_REOPEN`, `CONSOLE_WINDOWED`), the drains, the recipe cells (`TEGRA_SHELL_BASE/LEN/W/H/
// STRIDE/X/Y`), `tegra_shell_note`, `tegra_shell_live_id`, both `tegra_shell_remint` copies and
// the two board-named drains SR6 counted. What survives is the one cell the app model needs — the
// live row's id, registered — and the console's launch, which now RETRIES a transient decline
// (`dock::console_launch_service`, 32 passes) and names its count when it gives up.
//
// The x86 `wm::shell_remint` (WCSER-REMINT, `video/wm.rs`) is NOT a dock route and is NOT retired
// here: it is the wedge-rescue's ADOPTION of a corpse row after a render-core death, taken instead of
// close-and-recreate because `wm::close` on a corpse spins the F4 drain barrier against blit debt
// dead cores can never settle (flight 3: 21 s, then ABANDONED). Reported to the seat as a STOP.
//
// LINE-NEUTRAL: this block occupies exactly the lines the retired code did, so every
// `panic::Location` below it (JV1, and everything the knob-off x86 and virt images compile) is
// unmoved — measured by `./arroyo knoboff wc`.

/// APPPIN — the shell row's id, or `WIN_NONE` while no shell instance is live on the cascaded scene.
/// Registered with `wm::winid_register_holder` at the mint, so `wm::close` clears it on EVERY close
/// path (the disc, Quit, `wc_close_furniture`, a fixture) and it can never hand back a dead id
/// (SO1(b)'s registry). The pump's present reads it; the pump's quit arm keys on it.
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
static SHELLWIN_ROW: core::sync::atomic::AtomicU32 =
    core::sync::atomic::AtomicU32::new(unaos_kernel::video::wm::WIN_NONE);

/// APPPIN — the id and generation the LAST launch minted, kept ONLY for the quit line: by the time
/// the pump notices the row gone the table has freed it (and may have re-issued the slot), so the
/// `[dock] quit … win= gen=` witness names what was minted rather than re-reading the table.
/// `WIN_NONE` here means "no instance to quit" (never minted, or already reported).
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
static SHELLWIN_MINTED: core::sync::atomic::AtomicU32 =
    core::sync::atomic::AtomicU32::new(unaos_kernel::video::wm::WIN_NONE);
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
static SHELLWIN_GEN: core::sync::atomic::AtomicU32 = core::sync::atomic::AtomicU32::new(0);

/// APPPIN — the live shell row, or `WIN_NONE`. One relaxed load.
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
fn shellwin_row() -> unaos_kernel::video::wm::WinId {
...
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
fn shellwin_note_mint(id: unaos_kernel::video::wm::WinId) {
    use core::sync::atomic::Ordering;
    use unaos_kernel::video::{dock, wm};
    SHELLWIN_ROW.store(id, Ordering::Relaxed);
    wm::winid_register_holder(&SHELLWIN_ROW, "shellwin");
    SHELLWIN_MINTED.store(id, Ordering::Relaxed);
    SHELLWIN_GEN.store(wm::winid_gen(id), Ordering::Release);
    dock::app_launched(dock::PinnedApp::Shell, id, "console-pump", 1);
}

/// APPPIN — the pump found its row gone and has dropped the instance: report the quit ONCE, naming
/// the id and generation that were minted. Idempotent: a second call with nothing minted is silent.
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
fn shellwin_quit_note() {
    use core::sync::atomic::Ordering;
    use unaos_kernel::video::{dock, wm};
    let id = SHELLWIN_MINTED.swap(wm::WIN_NONE, Ordering::AcqRel);
    if id == wm::WIN_NONE {
        return;
    }
    dock::app_quit(dock::PinnedApp::Shell, id, SHELLWIN_GEN.load(Ordering::Acquire), "console-pump");
}

/// APPPIN — is a shell launch posted? The pump drains the dock's post itself, because it is the body
/// that owns the instance; `orin_render_service` drains only the CONSOLE's post now. One RMW.
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
fn shellwin_launch_owed() -> bool {
...
fn shellwin_scene_live() -> bool {
    unaos_kernel::video::menubar::enabled() && unaos_kernel::video::fbcon::console_is_routed()
}

/// APPPIN — should this keystroke be DROPPED? True when the scene is up and no shell instance is
/// live (`spal` is the window pal, `None` between a quit and the next launch): there is nothing to
/// type into, and the panel is the desktop's. See the block header.
#[cfg(all(target_arch = "aarch64", feature = "deskcascade"))]
fn shellwin_absent(spal: &Option<unaos_kernel::pal::TargetPal<'_>>) -> bool {
    spal.is_none() && shellwin_scene_live()
}

/// APPPIN — the Pi render body's shell QUIT and LAUNCH, folded into its mint arm's guard.
///
/// Called on every pass ahead of the `shell_id == WIN_NONE` term, on the SAME line (a fold, so the
/// knob-off `kernel8.img` line numbers are untouched). Two arms:
///
///  * **quit** — `shell_id` names a row that no longer carries `KERNEL_OWNER_DESKTOP` (the disc,
///    Quit or `wc_close_furniture` freed it through `wm::close`): report it, clear the stale id and
///    park the mint arm SHUT (`shell_declined = true`). Before this arc a closed shell left the dead
///    id in `shell_id` and the arm closed by accident; now it is closed on purpose, and the arm's
///    guard reads as "no instance, and none asked for".
///  * **launch** — a posted press. A live instance is raised (`focus_changed`, which also un-parks
///    a minimised shell), never doubled; otherwise the arm is OPENED (`shell_declined = false`) and
///    mints on this pass through `open_shell_window`, rebinding the whole tuple (the old store is
///    freed by that assignment). A genuine decline sets `shell_declined` again — final until the
///    operator's next press, which is the ask.
///
/// `winid_gen(*shell_id)` on the quit line is the slot's CURRENT generation: the row is already
/// freed, so if the slot was re-issued between the close and this pass the line names the new
/// tenant's generation. It is a witness field, not a fence; the fence is the owner test.
#[cfg(all(target_arch = "aarch64", feature = "baremetal", feature = "desktop_firmware"))]
fn shellwin_service_rearm(
    ...
    if dock::take_launch(dock::PinnedApp::Shell) {
        if *shell_id != wm::WIN_NONE {
            wm::focus_changed(wm::KERNEL_OWNER_DESKTOP);
            dock::app_raised(dock::PinnedApp::Shell, *shell_id, "render_service");
        } else {
...
/// APPPIN — the knob-off twin, so the fold at the call site costs the knob-off `kernel8.img`
/// nothing.
#[cfg(all(
    target_arch = "aarch64",
    feature = "baremetal",
...
/// APPPIN — drain the dock's CONSOLE launch. One relaxed RMW on a quiet pass. The whole launch — the
/// already-live raise, the mint through `fbcon::panel_console_window_open`, the retry of a
/// transient decline, the witness — is `dock::console_launch_service`, ARCH-NEUTRAL; this file needs
/// only a cfg wrapper, because the three call sites (`render_service`, `x86_render_service`,
/// `orin_render_service`) are each compiled in configurations where that module does not exist.
#[cfg(any(
...
#[inline(always)]
fn console_launch_drain(_who: &'static str) {}
...
fn virt_el0_start_maybe() {
    ...
    unaos_kernel::arch::sched::spawn_user("el0-hello", demo.hello, demo.sp, 0); #[cfg(feature = "virt_tick")] unaos_kernel::arch::sched::spawn_user("el0-spin", demo.spin, demo.sp, 0); // VIRTPREEMPT (orin 17) — the EL0 HALF of the preemption proof, appended on this line (zero source lines added). `demo.spin` is the M6e spinner `__user_prog_spin`: a long register-only EL0 loop that makes NO syscall until its final `sys_exit`, so nothing but a timer IRQ can take it off the CPU, and it writes no memory, which is why the Pi already shares `demo.sp` between it and `el0-hello` (this file's M6e block). Its preemption is read from `PRIO_EL0_DISPATCH` by the verdict at sched.rs's tail, NOT from the M6e `EL0_IRQS_AT_EL0` counter — the reason is at `aarch64_irq_handler`. Pinned to the boot core for the JV1 reason: the virt secondaries are parked at EL2 and `spawn_user` filters placement through `EL1_CORE_MASK`.
    unaos_kernel::arch::sched::spawn(
        "virt-el0-verdict",
