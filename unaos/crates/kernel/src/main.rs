...
fn tegra_rast_demo_maybe() {
    ...
    let mut screen = unaos_kernel::video::Screen::new(front_fb);
    unaos_kernel::rast_demo::run_mc(&mut screen); unaos_kernel::rast_demo::run(&mut screen); drop(screen); // A67: the `orin_rast_glass_post()` call that stood HERE has MOVED into `rast_demo::run`, onto the last composited frame, and this site is now bare. The reason is the repair: once the demo renders into a `wm` row, `run` closes that row before returning, so a `post` sample taken on this line reads the desktop the compositor repaints over the vacated box and reports NO-RAST-INK on every healthy boot. The sample must be taken while the row is still composited, and only `run` knows that instant. Line-NEUTRAL edit, in place: this is the rast-ON tegra arm (`tegra_rast_demo_maybe` is `#[cfg(all(tegra, rast, aarch64))]`), the knob-off twin at :7148 is untouched, and no line is added or removed — the panic `Location` byte-identity constraint. RAST-MC: the multi-core rung runs FIRST (1-core baseline + frame-pipelined pass, both unpaced) so the paced visible spin stays the last panel content of the boot; same-line per the zero-added-lines convention above. ORIN-RASTGLASS: the `post` glass read-back, fired the instant `run` returns — nothing is dispatched on this core in between, so it is the ONE sample that can establish whether the blit reached the scan-out at all (`late`, from the pump sweep, then says whether it survived). `drop(screen)` first so the double buffer's own `flush` has certainly landed before the panel is read back; also releases the back buffer to the heap ahead of ~2048 volatile VRAM reads. Same-line per the convention above.
}
