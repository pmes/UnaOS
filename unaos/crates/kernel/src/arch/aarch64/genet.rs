...
#[derive(Clone, Copy)]
pub struct NetInfo {
    /// The GENET NIC's MAC address.
    pub mac: [u8; 6],
    /// The configured interface IPv4 (leased or static-fallback).
    pub ip: [u8; 4],
    /// The default gateway the bring-up settled on.
    pub gw: [u8; 4],
    /// `true` if `ip` came from a DHCP lease, `false` for the static fallback.
    pub leased: bool,
    /// `true` if the GENET link reported up at bring-up.
    pub link_up: bool,
}
...
#[cfg(all(feature = "baremetal", target_arch = "aarch64"))]
pub use metal::netinfo;

/// PI-UI-3: no GENET on non-baremetal aarch64 builds — the shell's `netinfo` verb prints "no device".
#[cfg(not(all(feature = "baremetal", target_arch = "aarch64")))]
pub fn netinfo() -> Option<NetInfo> {
    None
}
...
mod metal {
    ...
    pub fn netinfo() -> Option<super::NetInfo> {
        use core::sync::atomic::Ordering::Relaxed;
        let (ip, leased) = crate::net_phy::settled_ipv4()?;
        let mac = GenetNic::mac().unwrap_or([0u8; 6]);
        let gw = NET_GW.load(Relaxed).to_be_bytes();
        Some(super::NetInfo { mac, ip, gw, leased, link_up: link_up() })
    }
