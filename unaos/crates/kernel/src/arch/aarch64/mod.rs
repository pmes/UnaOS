...
pub mod smpprobe; #[cfg(all(feature = "tegra", feature = "selfup"))] pub mod selfup_tegra; #[cfg(all(feature = "tegra", feature = "ga10bprobe1"))] pub mod ga10b_probe; #[cfg(all(feature = "tegra", feature = "tcuprobe"))] pub mod hsp_tegra; // ORIN-SELFUP: the self-update core (staged whole-ESP payload -> sha256 verify -> proven FAT write -> warm-reboot hook), wired into tegra_early_stop after ORIN-INSTALL-2; appended to THIS line for knob-off byte-identity (inline fns with Location-bearing ops live below). GA10B-PROBE1: the first read-only GA10B iGPU probe rung (BPMP rail gate -> risk-ordered announce-first BAR0 reads -> SYSTEM_OFF), its own gated pub mod also appended to THIS line for knob-off byte-identity (its Location-bearing ops live in the module). TCURX (orin 14): the Tegra234 HSP shared-mailbox / TCU RX read-only probe (`tcuprobe`, implies `tegra`), its gated pub mod appended to THIS line before the comment for the same knob-off byte-identity (P7: the attribute goes BEFORE the first `//`; after it the mod is prose).
