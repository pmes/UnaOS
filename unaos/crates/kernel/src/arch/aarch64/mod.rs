...
pub mod smpprobe; #[cfg(all(feature = "tegra", feature = "selfup"))] pub mod selfup_tegra; #[cfg(all(feature = "tegra", any(feature = "ga10bprobe1", feature = "ga10bprobe2", feature = "ga10bprobe3", feature = "ga10bprobe4a")))] pub mod ga10b_probe; #[cfg(all(feature = "tegra", feature = "tcuprobe"))] pub mod hsp_tegra; #[cfg(feature = "ga10bprobe5")] pub mod ga10b_fw; #[cfg(all(feature = "tegra", feature = "ga10bprobe5a"))] pub mod ga10b_ignite; // ORIN-SELFUP: the self-update core (staged whole-ESP payload -> sha256 verify -> proven FAT write -> warm-reboot hook), wired into tegra_early_stop after ORIN-INSTALL-2; appended to THIS line for knob-off byte-identity (inline fns with Location-bearing ops live below). GA10B-PROBE1: the first read-only GA10B iGPU probe rung (BPMP rail gate -> risk-ordered announce-first BAR0 reads -> SYSTEM_OFF), its own gated pub mod also appended to THIS line for knob-off byte-identity (its Location-bearing ops live in the module). TCURX (orin 14): the Tegra234 HSP shared-mailbox / TCU RX read-only probe (`tcuprobe`, implies `tegra`), its gated pub mod appended to THIS line before the comment for the same knob-off byte-identity (P7: the attribute goes BEFORE the first `//`; after it the mod is prose). GA10B-PROBE3 (orin 16): rungs 3/3b of the GA10B ladder live in the SAME `ga10b_probe.rs`, so the module gate's `any(...)` gained `ga10bprobe3` in place — no new line, no `Location` shift; `ga10bprobe3b` implies `ga10bprobe3` and therefore needs no term of its own here. GA10B-PROBE4 (orin 26): rungs 4a/4b live in the same file; `ga10bprobe4a` added to the `any(...)` in place, `ga10bprobe4b` implies it. GA10B-PROBE5 (orin-0912b): the rung-5 firmware LOADER `ga10b_fw` (`ga10bprobe5`, NOT tegra-gated — the QEMU-virt fixture runs it) and the rung-5a IGNITION `ga10b_ignite` (`tegra` + `ga10bprobe5a`) are two more gated pub mods appended to THIS line before the comment, same knob-off byte-identity reason.
