...
pub fn poweroff() -> ! { s5_ring_flush(); let _ = crate::drivers::xhci::ftdi_flush_sync(crate::arch::hw_wait_budget()); // S5DRAIN (trunk queue §5, 2026-09-12; LEDGER SO31) — THE DIRECT S5 ROUTE DRAINS TOO. `power::shutdown` flushes the staging ring before it calls here (SO31 part 3), but `video/crystal.rs`'s Shut Down and `video/instgui.rs` call this function DIRECTLY, so on those two routes S5 landed with the ring unflushed and every line still staged — the operator's evidence that the verb ran — died with the power. The flush belongs at the ONE place every x86 S5 route passes through, so the tree has one drain policy and not two. It is the FIRST statement because the `discover()` failure arm below prints and then parks in `hlt_loop`, which is likewise a context with no next print. Idempotent by construction: the `power::shutdown` route's own `power_drain` leaves the ring empty, so this call finds `lines=0` and says so, exactly as the reboot ladder's second drain already does (power.rs:170). ⚠ LINE-NEUTRAL fold onto the signature — `gas_space_name`, `table_checksum_ok`, `discover_reset`, `reset_settle`, `raw_witness`, `reset_report` and `reboot` all sit BELOW this line in this file, and one new source line would move every `panic::Location` record in them (LEDGER P7). The statement is first and the comment last, so nothing is commented out. Body: FILE-TAIL append. RBTDRAIN (rmbp-ledger A3) — THE SECOND STATEMENT, AND THE SAME ARGUMENT ONE BUFFER FURTHER ALONG. `s5_ring_flush` above empties the STAGING ring through `raw_write_str`, which on x86 is the 16550 at 0x3F8 alone; the 2012 rMBP has no such port, and its console is the FTDI cable, whose lines sit in `ftdi::RING` until the xHCI device-service pass carries them out — and S5 is a context with no next pass, exactly as S5DRAIN says of the staging ring. So THIS is what actually reaches a human on that laptop, and it belongs at the PORT for S5DRAIN's own reason: `power::shutdown` flushes in its own announce order, but `video/crystal.rs`'s Shut Down and `video/instgui.rs` call this function DIRECTLY, and before this statement those two routes reached S5 with the mirror ring unflushed altogether. It is also what carries `power::platform_shutdown`'s `[pwrshutoff] ftdi flushed …` tally out: that line is written INTO the ring one call earlier and had nothing left to take it out. AFTER `s5_ring_flush` and not before, so S5DRAIN's 'FIRST statement' property is unmoved and the staging drain still precedes everything; both run before the `discover()` failure arm's `hlt_loop` park and before the `interrupts::disable()` deeper in the body — which the FTDI TX pump requires, since it awaits completions through `crate::hlt()` and a `hlt` with IF clear never wakes. BOUNDED and NON-BLOCKING (`claim()`, never a lock; a `now_cycles` budget), and `(0,0,false)` on any board with no live FTDI console. LINE-NEUTRAL: appended to this same line, before its first `//`, so no `panic::Location` below moves.
...
pub fn reboot() -> ! { let _ = crate::drivers::xhci::ftdi_flush_sync(crate::arch::hw_wait_budget()); // RBTDRAIN (rmbp-ledger A3) — THE REBOOT PORT DRAINS THE CABLE, and it is the FIRST statement for two independent reasons. (1) PORT, NOT ROUTE: `power::reboot` flushes too, in its own announce order, but the shell verb is not the only way in here and a route that skipped this would reset with the whole ladder still sitting in the FTDI mirror ring — the same argument S5DRAIN made when it put `s5_ring_flush()` at `poweroff()`'s port, whose call sits folded onto ITS signature one screen up. (2) BEFORE THE MASK: the line below disables interrupts, and the FTDI TX pump awaits its bulk-OUT completions through `crate::hlt()` — a `hlt` with IF clear and no timer never wakes — so this flush must run while interrupts are still on or it would hang the very verb it exists to make honest. It is also what carries `power::reboot`'s `[pwrreboot] ftdi flushed …` witness to the wire: that line is written INTO the ring one call earlier and has nothing else left to take it out. BOUNDED and NON-BLOCKING by construction (`claim()`, never a lock; a `now_cycles` budget, never an unbounded wait), so a wedged xHCI pass costs this reboot its budget and never the machine, and a no-op returning `(0,0,false)` on any board with no live FTDI console — every x86 machine with a real 16550 included. ⚠ LINE-NEUTRAL fold onto the signature, for exactly the reason `poweroff`'s is: `s5_ring_flush` sits BELOW this line in this file and one new source line would move its `panic::Location` record (LEDGER P7). The statement is first and the comment last, so nothing is commented out.
