...
pub fn init(_dtb_addr: u64, _dtb_size: usize) {
    ...
    } #[cfg(all(target_arch = "x86_64", feature = "bar1wedge"))] crate::drivers::gpu::pcihealth::sticky_clear_post_enum(); // SECSTA2 (rmbp-ledger A1, shut-out register §6 P7) — the SECOND W1C clear of the root port's sticky status latches, and the ONE call site of it. HERE because this is the boot's "all buses enumerated" point: every enumeration walk this function performs is above it — `ehci::init`'s `0..=255` (line 838, and note that it is therefore BEFORE the at-arm clear inside the Kepler dispatch at line 1016, not after it as `PCIE-RP-RECOVERY.md` §11 assumed), `PciScanner::scan`, `full_census`, `detect_gpus`, `sdhc::probe` → `storage_inventory`, `ahci::probe`, and `init_network` → `find_device`, which is the last of them. AFTER the GPACE report rather than before it so the boot-pacing tiling is untouched: `span` is sampled at the top of that block, so a call placed above would land in `resid` and make a knob-ON boot's row disagree with every baseline taken without it. Same BSP, same sequential phase, same CF8 accessors the at-arm clear already writes from (`pcihealth.rs`, `bw_arm`); guarded on `PCIH_READY`, so a machine with no Kepler (q35, every non-kepler boot) prints nothing. ⚠ LINE-NEUTRAL append, for the same reason the AHCI hook above is one: `panic::Location` records embed line numbers, knob-off x86 image byte-identity is this rung's stated invariant, and a cfg'd-OFF block still shifts every line below it. The statement goes BEFORE any `//` on this line — after it, it is a comment that compiles nothing and the gates stay green.
}
