...
pub fn service_dump() {
    ...
    #[cfg(target_arch = "x86_64")]
    crate::arch::syscall::clock_x1_poll(); #[cfg(target_arch = "x86_64")] crate::arch::memory::fb_wc_witness(); #[cfg(target_arch = "x86_64")] crate::drivers::xhci::ftdi::late_verdict(); // PHASE31WIT/FBWCWIT (rmbp B112/B114) — two witnesses that MUST be spoken after the console carrier exists, riding here for CLOCK-X1's stated reason and no other: this fn is the one call all three x86 service lanes make UNGATED, so it reaches the media build that actually boots on the bench. Both are latched one-liners (a relaxed load per pass once said). `fb_wc_witness` (renamed from `bar1exp_witness` by FBWCWIT, which gave the default WC arm the same treatment) replaces a `serial_println!` that sat at `BPACE: fb-wc t=0ms` — `kernel_main`'s first statement — and was never once observed on the rMBP, whose only carrier is the drop-oldest FTDI ring; `late_verdict` re-says the `:: FTDI-CAP:` integrity line that `emit_verdict` puts AHEAD of the replay, where a late-attaching `cat /dev/ttyUSB0` never sees it (0 hits in the 2026-09-16 capture, both boots, while the ring was pinned at capacity). The ledger this fn prints is itself the proof of reachability: `BPACE: total gui=` lands at 23534 ms in the very capture that lost the t=0 lines. ⚠ LINE-NEUTRAL append, and every statement precedes the first `//` (LEDGER P7).
