...
pub fn mirror_service() {
    ...
    #[cfg(feature = "witness")] pwrdrain_selftest(); #[cfg(all(target_arch = "x86_64", feature = "witness"))] s5drain_selftest(); #[cfg(feature = "witness")] serwire_selftest(); // SERWIRE (SO45) — LAST, after every fixture that READS the ring, for PWRDRAIN's own stated reason: it fills the ring and empties it again. It also zeroes the SERWIRE odometer on entry, so a fixture running after it would be measured against a span this one opened. ⚠ LINE-NEUTRAL append, before the line's first `//`; the body is a FILE-TAIL append. S5DRAIN (trunk queue §5, 2026-09-12) — PWRDRAIN's twin for the x86 route that does NOT go through `power.rs`: `video/crystal.rs`'s Shut Down and `video/instgui.rs` call `arch::acpi_power::poweroff()` directly. x86-only because the defect is: on aarch64 the desktop's Shut Down is `power::crystal_shutdown`, which has drained since SO31. Last, and after PWRDRAIN, for PWRDRAIN's own stated reason — it fills the ring to SLOTS and empties it again, so it must not run before a fixture that reads the ring. ⚠ LINE-NEUTRAL append; the body is a FILE-TAIL append, so no `panic::Location` in this file moves.
}
