...
/// | `masked_b` | of those, the bytes written with interrupts off — the currency of the dark window, at 86.8 us each. Bounded by `MASKED_BYTE_BUDGET` per masked print. **NON-ZERO ON A BUSY SPAN BY DESIGN**, and said here because the field invites the opposite reading: the invariant that a `_print` emits ZERO bytes behind the mask is [`sertx_selftest`]'s, asserted on the `:: SERIALTX: … masked_b=0 … -> PASS ::` line, not on this one. SERTXPIN's `masked_b=0` in the spec pin therefore asserts only that SOME span in the boot reached zero — which pre-SERIALTX no span with `prints>0` could, because every print wrote its own line synchronously under the mask. ⚠ SAME-LINE fold. |
...
/// | `tap_max` / `tap_sum` | TAPSMAX — the SAME two quantities, BY TAP: `fbcon:<us>,ftdi:<us>,tste:<us>,rec:<us>`. `taps_us_max=` is one number over four sinks and so cannot name a suspect; this pair does. Read `tap_max` first — a dark window is a MAXIMUM — and read `tap_sum` beside it to tell a steady cost from a tail, exactly as `masked_us_mean` is read against `masked_us_max`. `rec` is the `flightrec` tap (`UNAOS.LOG`). Each is a LOWER BOUND under multi-core contention, by [`TapCounters::charge_span`]'s stated one-sidedness. SERTXPIN (rmbp-ledger B174) — **this pair is PINNED, and until B174 nothing on any lane pinned any of this line** (B161 recorded the gap): `scripts/specs/x86-default.spec` and `x86-wc.spec` each REQUIRE the whole `[sertx]` shape with `tap_max=`/`tap_sum=` asserted as a FOUR-NAME ORDERED TABLE, character for character in the names and the commas, because the order is [`taps`]' order and that is what lets a reader line them up against the `:: SERWIT-2 tap …:` lines with no lookup. Every NUMBER is left open — they are wall on a shared bench. Anyone adding, dropping or reordering a tap re-pins both specs in the same commit. ⚠ SAME-LINE fold. |
