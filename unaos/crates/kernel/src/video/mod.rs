...
pub mod theme; pub mod keymap; pub mod clipboard; pub mod termsel; // TERMSEL — the terminal's SELECTION over the editable shell line (`LineSel`, the one `[termsel]` witness site, and the fixture), declared beside `clipboard` because `clipboard::terminal_action` is its consumer; unconditional for the reason `clipboard` is (the console, compiled on every image, holds a `LineSel`). ⚠ FOLDED onto this line, CODE FIRST (A10FIX). // APPCLIP (R61) — the CLIPBOARD and the delivery of the edit actions keymap.rs resolves, declared beside the two modules it sits between: `keymap.rs` says what a chord MEANS, `theme.rs` holds the rows, and this holds the one buffer those meanings move text through plus `terminal_action`, the shell line editor's consumer. Unconditional, for the reason `keymap` is: both HID decoders and both arches' `pack_input` call into it from unconditional code, so a knob-gated clipboard would be a compile error on the default image. ⚠ FOLDED onto the `theme`/`keymap` line and given none of its own — this module root is lexed into every image and a line added here moves every `panic::Location` below it (LEDGER P7); CODE FIRST, then the prose (A10FIX). // KEYMAP (R60) — the desktop's key BINDINGS, declared beside the theme that OWNS them, because that is the relationship: a chord is a property of the theme exactly as a colour is. `keymap.rs` holds the mechanism (the `Action` enum, `Binding`, `Table`, and the ONE resolver both HID decoders ask instead of testing chords as literals); `theme.rs` holds the rows. Unconditional, for the reason `theme` is: the decoders that call the resolver are unconditional code, so a knob-gated resolver would be a compile error on the default image. ⚠ FOLDED onto the `theme` line and given none of its own — this module root is lexed into every image and a line added here moves every `panic::Location` below it (LEDGER P7); LAWS' "a cfg'd-out `pub mod` declaration is the exception" does not apply, this declaration is not cfg'd.
