...
// EL0 — the identical path the operator drives with `run /apps/VUG.ELF`.
//
// WHAT IT DOES
...
//   * DETACHED (`bg /apps/VUG.ELF` — the desktop spawn): UNBOUNDED. It exits on ESC or `kill`, never on
...
pub extern "C" fn _start() -> ! {
    ...
    // DETACHED, i.e. by `bg /apps/VUG.ELF` rather than `run`. A detached vug has no operator to press ESC
