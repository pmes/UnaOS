...
fn test_soul_logic() {
    ...
    let file1_id = fs
        .create_file(root_id, "happy.txt".to_string())
        .expect("Failed to create file1");
    let file2_id = fs
        .create_file(root_id, "sad.txt".to_string())
        .expect("Failed to create file2");
    let file3_id = fs
        .create_file(root_id, "neutral.txt".to_string())
    ...
    fs.set_attribute(
        file1_id,
        "emotion".to_string(),
        AttributeValue::String("happy".to_string()),
    )
    .expect("Set attr failed");
    fs.set_attribute(
        file2_id,
        "emotion".to_string(),
        AttributeValue::String("sad".to_string()),
    )
    .expect("Set attr failed");

    // Vector attribute (Small)
    let vec_small = AttributeValue::Vector(vec![0.9, 0.1]);
    fs.set_attribute(file1_id, "embedding".to_string(), vec_small.clone())
        .expect("Set vec failed");
    ...
    fs.set_attribute(file3_id, "embedding".to_string(), vec_large.clone())
        .expect("Set large vec failed");

    // 4. Verify Get Attribute
    let attr1 = fs
        .get_attribute(file1_id, "emotion")
        .expect("Get attr failed")
        .unwrap();
    assert_eq!(attr1, AttributeValue::String("happy".to_string()));

    let attr3 = fs
        .get_attribute(file3_id, "embedding")
        .expect("Get large attr failed")
        .unwrap();
